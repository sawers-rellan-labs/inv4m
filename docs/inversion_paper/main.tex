...
We acknlowledge that a formal test of local adaptation would require reciprocal transplant experiments including highland environments where \invfour is nearly fixed.

\subsection*{Transcriptomic effects of \invfour are localized to the introgressed haplotype}

In the Crow et al.\ \cite{crow2020} growth chamber dataset, no MDS axis was diagnostic for \invfour genotype (S3 Fig in \cite{crow2020}).
In our field experiment, the third MDS dimension classified genotypes with $r = 0.92$ (Fig~\ref{fig:transcriptome}B), identifying a transcriptomic axis that separates \invfour from control samples against the larger effects of phosphorus treatment and leaf developmental stage (dimensions 1--2; \cite{rodriguez2026phosphorus}).
The 465 DEGs associated with this genotype axis concentrated within the 39 Mb shared introgression on chromosome 4, with 97-fold enrichment relative to the rest of the genome (Table~\ref{tab:DEGs_distro}).
In contrast, DEGs associated with leaf developmental stage distributed across all ten chromosomes (Fig~\ref{fig:transcriptome}D versus E).

This localized pattern is not a trivial expectation.
Chromosomal inversions can disrupt gene expression through position effects at breakpoints, where juxtaposition of euchromatic genes with heterochromatic sequences leads to stochastic silencing \cite{elgin2013}.
However, DEG density did not differ between \invfour proper and the flanking introgressed regions (OR $= 1.02$, $p = 0.95$; Table~\ref{tab:DEGs_distro}), indicating that the transcriptional perturbation tracks the introgression boundaries rather than the inversion breakpoints.
This pattern is consistent with expression differences driven by sequence divergence between the highland and B73 haplotypes rather than by the structural rearrangement itself.
Said et al.\ \cite{said2018} reached the same conclusion experimentally in \textit{Drosophila}: synthetic inversions engineered on controlled genetic backgrounds produced no differential expression, while natural inversions with accumulated linked variation produced widespread DE.
In \textit{Arabidopsis}, CRISPR-induced inversions that relocated heterochromatic sequences into euchromatin produced no detectable changes in epigenetic marks or gene expression at the rearranged segments \cite{khosravi2025}.
In \textit{Mimulus}, expression divergence within a locally adaptive inversion accumulated gradually through linked mutations rather than breakpoint effects \cite{kollar2025}.

Because the DEG signal tracks the introgression rather than the inversion, we cannot attribute specific expression changes to \invfour as opposed to the broader introgressed haplotype from the differential expression analysis alone.
Introgressed segments are expected to produce local DE proportional to sequence divergence from the recurrent parent, as demonstrated empirically in \textit{Solanum} introgression lines \cite{guerrero2016}.
We tested this expectation directly by comparing CDS percent identity between B73 and Mi21 orthologs for 254 expressed genes in the introgression. CDS divergence predicted |logFC| (additive model adj R² = 0.104, divergence p = 2.4e-06). Genes inside Inv4m were more diverged than flanking genes (median 1.1\% vs 0.75\%, Wilcoxon p = 0.003), consistent with suppressed recombination preserving ancestral mexicana divergence.
After accounting for divergence, Inv4m genes showed smaller absolute expression changes than flanking genes (region p = 0.002), indicating that the inversion dampens rather than amplifies expression divergence.
Twenty-one genes had identical CDS and were all DE; 20 of 21 were flanking (Fisher p = 0.00013), consistent with trans-regulatory effects from divergent loci elsewhere in the introgression.
We address this confounding in subsequent sections through trans-coexpression network analysis, which identifies genes within the introgression whose expression changes propagate to targets elsewhere in the genome.
...
\subsection*{Coexpression networks connect JMJ to growth-related genes}
...
WGCNA analysis of the PA2022 leaf transcriptome identified 13 modules, of which 12 showed significant connectivity loss in \invfour relative to CTRL (one-sample $t$-test, $\textit{FDR} < 0.05$; Table~\ref{tab::preservation}).
JMJ genes functioned as network hubs in CTRL ($k_{Within} = 3.17$ to $3.33$) but became peripheral in \invfour ($k_{Within} = 0.59$ to $0.61$; Fig~\ref{fig:wgcna}C), consistent with the dosage reduction disrupting their coexpression relationships.
The pink module, enriched for ribosome biogenesis and growth regulation (Table~\ref{tab::pink_module}), showed the most significant connectivity reduction (mean $\Delta k_{Within} = -1.36$, $p_{adj} = 2.6 \times 10^{-6}$) despite moderate structural preservation ($Z_{summary} = 5.8$).
The trans coexpression network independently linked JMJ genes to cell proliferation factors including \textit{pcna2} and \textit{sec6} (Fig~\ref{fig::transnetwork}A), with edges assigned only when JMJ was the strongest correlated regulator within the 15 Mb inversion (see Methods).
...
These network observations are consistent with a model in which reduced JMJ dosage alters H3K4 methylation patterns, disrupting coexpression of growth-related genes and contributing to the meristem and whole-plant phenotypes we observed.
...
Chromosomal inversions maintain coadapted allele complexes by suppressing recombination \cite{kirkpatrick2006b, dobzhansky1970}.
...
Coexpression network analysis offers a way around both constraints: by comparing network architecture between genotypes, we can identify which regulatory relationships are reconfigured and which genes occupy positions --- such as network hubs --- where dosage changes would propagate through the transcriptome, regardless of whether the phenotypic effects in a given environment are beneficial or deleterious.
...
That this reconfigured network is associated with earlier flowering --- an adaptive phenotype even in temperate conditions --- suggests that the coadapted allele complex retains functional coherence outside its native environment.

A growing body of work across taxa establishes that megabase-scale inversions alter gene regulation and produce phenotypic consequences, the two prerequisites for adaptive relevance.
...
Expression divergence within this \textit{Mimulus} inversion evolved gradually through accumulated mutations rather than breakpoint effects \cite{kollar2025}, paralleling our finding that DEGs are not enriched inside versus outside \invfour (OR $= 1.02$, Table~\ref{tab:DEGs_distro}), consistent with trans-regulatory effects from hub genes rather than position effects.
In sunflower, inversions contribute disproportionately to parallel dune ecotype divergence, with seed size QTLs clustering within inversions more often than expected by chance \cite{huang2025}.
...
\invfour extends these findings by connecting the two levels at which inversions act: coordination among genes within the inversion and phenotypic effects on the individual in its environment.
The expression changes we observe are not buffered as in yeast --- they correlate with quantitative variation in flowering time, plant height, and meristem geometry across environments.
...
If the functional coordination captured by coexpression networks reflects the coadaptation that theory predicts for inversion-maintained haplotypes \cite{kirkpatrick2006b, dobzhansky1970}, then epistatic regulators --- genes whose dosage changes reshape entire modules --- are strong candidates for mediating the phenotypic effects of the inversion.
The JMJ histone demethylase cluster, with its five-to-one copy number difference between B73 and highland genomes and its hub-to-peripheral transition in \invfour, fits this profile.
We stress that this remains a hypothesis: we cannot yet distinguish whether JMJ drives the network reconfiguration or whether both JMJ downregulation and network changes are parallel consequences of the introgression.
But it generates a testable prediction --- that restoring JMJ dosage should rescue network connectivity --- and motivates the functional follow-up experiments outlined below.
...
Calfee et al.\ \cite{calfee2021-mr} identified \textit{Inv9f} as a second highland-adaptive inversion with independent clinal introgression, suggesting that multiple structural variants act in concert for highland adaptation.
...
\begin{tabular}{@{}llrrl@{}}
\toprule
\textbf{Gene ID} & \textbf{Locus} & \textbf{$-\log_{10}$(\textit{FDR})} & \textbf{$\log_2$FC} & \textbf{Description} \\ \midrule
\multicolumn{5}{l}{\textbf{\invfour: Downregulated}} \\ \midrule
Zm00001eb191790 & \textit{jmj2} & 21.9 & $-3.49$ & JUMONJI-transcription factor 2 \\
Zm00001eb191820 & \textit{jmj4} & 19.7 & $-2.84$ & JUMONJI-transcription factor 4 \\
Zm00001eb189910 & \textit{metrs1} & 17.5 & $-2.86$ & Methionyl-tRNA synthetase \\
Zm00001eb192850 &  & 17.5 & $-1.98$ & SUMO-activating enzyme subunit uba-2 ... \\
Zm00001eb192630 &  & 15.8 & $-1.69$ & CCT-alpha \\
Zm00001eb188070 & \textit{arf1} & 15.8 & $-2.78$ & ADP-ribosylation factor \\
Zm00001eb188750 & \textit{hsp70-17} & 14.2 & $-6.48$ & Heat shock 70 kDa protein 17 \\
Zm00001eb190240 &  & 14.2 & $-9.16$ &  \\
Zm00001eb188240 & \textit{ropgef14} & 13.6 & $-2.98$ & Rop guanine nucleotide exchange facto... \\
Zm00001eb193260 & \textit{trxl2} & 13.2 & $-6.10$ & Thioredoxin-like 3-1, chloroplastic \\
Zm00001eb190350 &  & 13.0 & $-1.83$ & Ankyrin repeat-containing protein \\
\midrule
\multicolumn{5}{l}{\textbf{\invfour: Upregulated}} \\ \midrule
Zm00001eb264460 & \textit{ychf} & 16.9 & $+6.50$ &  \\
Zm00001eb194380 & \textit{sec6} & 16.5 & $+2.87$ & Exocyst complex component Sec6 \\
Zm00001eb190670 & \textit{zfrvt} & 15.6 & $+1.97$ &  \\
Zm00001eb191990 & \textit{pig3} & 15.0 & $+1.97$ & Quinone oxidoreductase PIG3 \\
Zm00001eb241410 & \textit{etfa} & 14.0 & $+4.46$ & Electron transfer flavoprotein subuni... \\
Zm00001eb213070 & \textit{gbp1} & 13.7 & $+8.96$ & GTP-binding protein-related \\
Zm00001eb195370 &  & 13.6 & $+1.63$ &  \\
Zm00001eb041890 & \textit{kan1} & 13.6 & $+1.68$ & KANADI1 \\
Zm00001eb189920 & \textit{aldh2} & 13.6 & $+2.70$ & aldehyde dehydrogenase2 \\
Zm00001eb190750 & \textit{jmj21} & 13.5 & $+2.28$ & JUMONJI-transcription factor 21 \\
Zm00001eb189580 &  & 13.2 & $+7.38$ &  \\
Zm00001eb182850 & \textit{mybsant} & 12.4 & $+6.96$ &  \\
Zm00001eb112470 & \textit{engd2} & 12.3 & $+6.14$ &  \\
Zm00001eb187280 & \textit{gpm458} & 11.2 & $+10.47$ & Oil body-associated protein like \\
\midrule
\multicolumn{5}{l}{\textbf{Leaf$\times$\invfour}} \\ \midrule
Zm00001eb194380 & \textit{sec6} & 3.1 & $-0.55$ & Exocyst complex component Sec6 \\
\bottomrule
\end{tabular}
\end{table}
