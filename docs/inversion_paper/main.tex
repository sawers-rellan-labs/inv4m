...
The dataset-specific edges are statistically well-supported in our experiment (FDR-corrected Pearson correlation $p < 0.05$) and their absence from MaizeNetome indicates condition-specificity rather than lack of biological support.
...
\subsection*{\invfour introgression effects on gene expression are local and concordant with prior growth chamber data}

As established in our companion study \cite{rodriguez2026phosphorus}, the first two dimensions of the gene expression MDS space reflected tissue differences (leaf stage, phosphorus treatment) among samples in our field experiment.
In contrast, the third MDS dimension was diagnostic for
\invfour vs CTRL genotype ($r = 0.92$;
Fig.~\ref{fig:transcriptome}B).
Although \cite{crow2020} identified cis effects of \invfour introgression shared between Mi21 and PT, a diagnostic gene expression MDS axis for the \invfour karyotype was not reported.
The \invfour DEGs concentrated within the 39 Mb shared introgression on chromosome 4 (272/608), with 97-fold enrichment relative to the rest of the genome (193/24011)(Table~\ref{tab:DEGs_distro}).
In contrast, leaf developmental stage DEGs distributed homogeneously throughout the genome.
However, DEG density did not differ between \invfour proper and the flanking introgressed regions (OR $= 1.02$, $p = 0.95$; Table~\ref{tab:DEGs_distro}), 
...
Experimental evidence across taxa supports this interpretation: synthetic inversions engineered on  controlled genetic backgrounds in \textit{Drosophila} produced no differential expression, while natural inversions with accumulated linked variation produced widespread DE \cite{said2018}.
Similarly CRISPR-induced inversions in \textit{Arabidopsis} similarly left epigenetic marks and gene expression unchanged \cite{khosravi2025}.
...
Introgressed segments are expected to produce local DE, most likely downregulation, as demonstrated in \textit{Solanum} introgression lines \cite{guerrero2016}, but  upregulation has recently been reported in maize accombapnied by reduced sRNA silencing \cite{minow2021}.
Crow et al.\ \cite{crow2020} showed greater sequence divergence among Mexican traditional varieties inside \invfour than outside, consistent with suppressed recombination.
Our coding sequence comparison between B73 and Mi21 orthologs confirms this pattern at the gene level: genes inside \invfour were more diverged than flanking genes (median 1.1\% vs 0.75\%, Wilcoxon $p = 0.003$; Fig~\ref{fig:divergence}).
CDS divergence was a significant but weak predictor of $|\log_2\text{FC}|$ (additive model adj $R^2 = 0.104$, divergence coefficient $p = 2.4 \times 10^{-6}$), and the relationship was driven largely by a few highly divergent genes with large expression changes.
After accounting for divergence, Inv4m genes showed smaller absolute expression changes than flanking genes (region coefficient p = 0.002), indicating that the inversion dampens rather than amplifies expression divergence.
...
We address this confounding in subsequent sections through trans-coexpression network analysis, which identifies genes within the introgression whose expression changes more tightly correlate to targets elsewhere in the genome.

Our results overlap with the growth chamber study of Crow et al.\ \cite{crow2020}, who identified DEGs associated with \invfour in B73 near-isogenic lines in a previous generation of the Mi21 and PT NIls.
They reported 27 unique candidate genes within the \invfour haplotype, each classified as either consistently differentially expressed, correlated in trans, or encoding a transcription factor.
After mapping v4 to v5 gene identifiers, 20 of these were also DEGs in our field study (FDR $< 0.05$).
Of the 20 shared DEGs, 13 were downregulated and 7 upregulated.
The shared genes span several functional categories.
In cell proliferation, \textit{pcna2} ($+2.99$, correlated in trans), and in growth, \textit{sec6} ($+2.87$, consistently differentially expressed), were both upregulated.
...
Among chromatin regulators, both studies identified the JMJ histone demethylase cluster (listed as ``Putative lysine-specific demethylase JMJ16'' in their v4 annotation): \jmjii ($\log_2\text{FC} = -3.49$), \jmjiv ($-2.84$), and a separate JMJ-domain transcription factor (\textit{jmj21}, $+2.28$).
...
JMJ genes functioned more as network hubs in CTRL ($k_{Within} = 3.17$ to $3.33$) but became peripheral in \invfour ($k_{Within} = 0.59$ to $0.61$; Fig~\ref{fig:wgcna}C), consistent with the dosage reduction disrupting their coexpression relationships.
The pink module, where jmj2 belongs, is enriched for ribosome biogenesis and growth regulation (Table~\ref{tab::pink_module}), and showed the most significant connectivity reduction (mean $\Delta k_{Within} = -1.36$, $p_{adj} = 2.6 \times 10^{-6}$) despite moderate structural preservation ($Z_{summary} = 5.8$).
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=0.95 \linewidth]{figs/jmjcluster.png}
