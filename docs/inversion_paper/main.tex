...
Spatially-corrected phenotype values (per-plot residuals from SpATS spatial models fit with \texttt{repId} as a nuisance fixed factor) compared between CTRL (yellow) and \invfour (purple) genotypes.
Stage-2 contrasts were computed as \texttt{lm(corrected $\sim$ Genotype $\times$ Treatment)} with marginal Genotype contrasts (pooled over phosphorus level) extracted via \texttt{emmeans}.
P-values are BH-FDR adjusted across the single pool of all (trait $\times$ contrast) rows.
...
\textbf{(C) 21 DAP Seedling SAMs.} Representative differential interference contrast micrographs of cleared vegetative shoot apical meristems from 21-day-old seedlings grown in controlled environment chambers; yellow lines indicate height ($h$) and radius ($r$) used for quantification.
Scale bars: 100 $\mu$m.
\textbf{(D) SAMs.} SAM shape descriptors derived from C: height-to-radius ratio ($h/r$) and shape factor ($h/r^2$, nm$^{-1}$), tested with two-sided Welch \textit{t}-tests.
