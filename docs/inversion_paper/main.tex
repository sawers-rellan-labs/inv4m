...
Weighted gene coexpression network analysis shows that the introgression reduces intramodular connectivity in a genotype specific manner for 5 of 13 DEG coexpression modules (label permutation test, FDR $<$ 0.05), with JMJ genes losing hub connectivity in the pink module.
...
By introgressing the highland inverted haplotype from the Mexican traditional variety Mi21 into B73 through eight generations of backcrossing, generating Near Isogenic Lines (NILs), we reduced the confounding Mi21 linkage drag to 24 Mb around the 15 Mb inversion.
...
The \invfour region has also been associated with phosphorus starvation response through the \textit{phos2} gene \cite{ma2021}.
To test this effect, we evaluated \invfour NILs in the Pennsylvania 2022 field season under different levels of phosphorus and the results are presented in a companion study~\cite{rodriguez2026phosphorus}.
We used RNA-seq from the Pennsylvania 2022 field leaves to identify differentially expressed genes and applied weighted gene coexpression network analysis (WGCNA) and trans-regulatory network reconstruction to characterize the regulatory architecture of the \invfour effect.


In this study, we used long read genome assemblies of highland materials and a Mi21 NIL to precisely determine \invfour breakpoints, which overlap chromosomal knob repeat arrays, consistent with an ectopic recombination origin.
We show that \invfour modulates flowering and plant height across environments, with genotype-by-environment interactions congruent with local adaptation.
In a growth chamber assay, shoot apical meristem morphology confirmed that \invfour NILs have larger, more elongated meristems, a developmental pattern associated with faster flowering \citep{danilevskaya2008, wu2008, leiboff2015, thompson2015}.
...
This evidence suggests that \invfour regulates growth by rewiring gene networks involved in growth regulation probably through chromatin modification.
These molecular mechanisms are a plausible pathway for the reported adaptive effect of \invfour in highland conditions, where it promotes earlier flowering.
...
\textbf{(A)} PA2022 NIL field trial; emmeans contrasts on stage 2 linear models, FDR adjusted within trial.
\textbf{(B)} PA2024 hybrid field trial; emmeans contrasts as in A, FDR adjusted within trial.
...
\textbf{(D)} SAM dimensions: height, height to radius ratio, and shape factor; one tailed Welch \textit{t}-tests (Inv4m $>$ CTRL), FDR adjusted.\\
...
At the Pennsylvania 2022 site (PA2022), our spatially-corrected phenotype analysis showed that NILs carrying the highland inversion allele flowered $1.29 \pm 0.26$ days earlier to anthesis ($\text{FDR} = 1.6 \times 10^{-4}$) and $1.05 \pm 0.22$ days earlier to silking ($\text{FDR} = 1.6 \times 10^{-4}$) than control lines, and grew $6.48 \pm 1.12$ cm taller ($\text{FDR} = 1.4 \times 10^{-5}$) (Fig~\ref{fig:phenotypes}A).
The harvest index and stover dry weight at harvest also showed modest but significant changes (FDR $< 0.05$), while total kernel weight in this temperate trial did not differ between genotypes (Fig~\ref{fig:phenotypes}A, TGW panel).
Phosphorus treatment was modeled as a fixed factor in the stage-2 linear model and contrasts averaged over P levels (see Methods).
...
To test whether the \invfour flowering time effect generalizes across donor genetic backgrounds, we analyzed an independent BZea NIL panel grown at the Central Crops Research Station (Clayton, NC; NC2025) in which the \invfour bearing segment had been introgressed into B73 from multiple teosinte donor accessions spanning four \textit{Z.~mays} ssp.\ \textit{mexicana} ancestry groups (Chalco, Durango, Mesa-Central, Nobogame) and \textit{Z.~mays} ssp.\ \textit{huehuetenangensis} (Huehuetenango).
After the both arms BC1 filter the panel comprised 393 plots from 218 NILs across 35 BC1 families; Balsas parviglumis, \textit{Z.~luxurians}, and \textit{Z.~diploperennis} donor lineages were excluded because they do not carry the mexicana derived \invfour haplotype (no donor arm at the \invfour locus).
A linear mixed model with strictly nested random effects (ancestry / BC2 family / NIL; see Methods) recovered the flowering time signal we observed in the MI21 single donor panel: \invfour donors silked $0.88 \pm 0.31$ days earlier ($p = 0.004$, FDR $= 0.013$) and reached anthesis $0.61 \pm 0.27$ days earlier ($p = 0.022$, FDR $= 0.033$) than the recurrent B73 haplotype arm within the same NIL line, with within BC2 permutation $p$ values agreeing within rounding (Supplementary Fig.~\ref{fig::zeal_inv4m}A,B).
...
Plant height showed a positive but non significant donor effect in the BZea panel ($+1.6 \pm 2.7$ cm, $p = 0.55$; Supplementary Fig.~\ref{fig::zeal_inv4m}C,D), directionally consistent with the Pennsylvania environment height increases observed in the MI21 panel but with insufficient resolution to detect a $\sim$1.6 cm effect against the donor lineage scatter ($\sim$15 cm within arm SD) introduced by the multi donor design.
...
To investigate whether these macroscopic phenotypes were associated with changes in meristem organization, we examined shoot apical meristems (SAMs) from seedlings grown two weeks in controlled environment chambers (Fig~\ref{fig:phenotypes}C,D).

\begin{figure*}[!ht]
\centering
\includegraphics[width=0.7\linewidth]{figs/RNAseq.png}
...
As a topological control, we repeated the entire consensus pipeline on an orthogonal ``Leaf Gradient'' reference network built from a curated set of 218 panicoid-grass photosynthesis and senescence genes (163 and 55, respectively) characterized by leaf-developmental expression dynamics rather than by genotype-driven differential expression (\cite{ojedarivera2026}; Methods, Table~\ref{tab::preservation_lg}).
The 5 Leaf Gradient modules (sky, peach, jade, coral, lavender) all reached stable bootstrap support (LFP $\geq 0.81$; Fig.~\ref{fig::module_support}) and showed strong rank preservation ($Z_{connectivity}$ 4.5 to 10.7), yet \textit{none} reached significance under the genotype-label permutation test (all FDR $>$ 0.2; Fig.~\ref{fig:wgcna}B, bottom facet; Table~\ref{tab::preservation_lg}), even though their raw mean $\Delta k_{Within}$ values were large (from $-1.36$ to $-13.82$).
...
Hub gene analysis showed that \jmjiv, a top-connected gene in CTRL ($k_{Within} = 3.17$), retained a similar rank within the pink module but lost 81\% of its absolute connectivity in \invfour ($k_{Within} = 0.61$; Fig~\ref{fig:wgcna}C).
...
\textbf{(B)} Permutation test of intramodular connectivity loss in \invfour relative to CTRL.
...
$y$-axis labels show module color and sample size ($n$).
FDR-corrected $p$-values are shown in red next to modules with significant connectivity loss.
Text to the right of each ridge shows the top reduced Gene Ontology Biological Process term for that module.
...
Gene symbols are shown in italics where available; \jmjii is highlighted in bold italic.}
\label{fig:wgcna}
\end{figure*}
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=0.9 \linewidth]{figs/jmjcluster.png}
...
In the PA2022 NIL field trial, plants carrying \invfour flowered earlier ($-1.29 \pm 0.26$ days to anthesis, $\text{FDR} = 1.6 \times 10^{-4}$) and grew $6.48 \pm 1.12$ cm taller ($\text{FDR} = 1.4 \times 10^{-5}$) than controls (Fig~\ref{fig:phenotypes}A).
...
In the PA2024 NIL-derived hybrid trial, by contrast, \invfour-carrying hybrids produced significantly greater grain yield: $+194 \pm 52$ g total grain weight ($\text{FDR} = 6.0 \times 10^{-3}$) and $+841 \pm 254$ kernels per plot ($\text{FDR} = 1.2 \times 10^{-2}$) over the \texttt{Inv4\_B73} control (Fig~\ref{fig:phenotypes}B), suggesting that \invfour's effect on grain biomass is buffered in the NIL background but expressed when the inversion is heterozygous against a complementary parent.
...
SAM measurements from seedlings grown two weeks in controlled environment chambers showed that \invfour plants have elongated meristems: SAM height increased $+9.65 \pm 5.50$ $\mu$m and the height to radius ratio and shape factor ($h/r^2$) were also elevated (all one tailed Welch \textit{t}-tests with directional hypothesis Inv4m $>$ CTRL, BH-FDR $<$ 0.05; Fig~\ref{fig:phenotypes}C,D).
...
Introgressed segments are expected to produce local DE, most likely downregulation, as demonstrated in \textit{Solanum} introgression lines \cite{guerrero2016}, but  upregulation has recently been reported in maize accompanied by reduced sRNA silencing \cite{minow2021}.
...
The JMJ cluster, like the rest of the pink module, experienced substantial absolute connectivity loss in \invfour ($k_{Within}$ fell from $3.17$--$3.33$ in CTRL to $0.59$--$0.61$ in \invfour; Fig~\ref{fig:wgcna}C) despite the module's rank structure being partially preserved ($Z_{connectivity} = 6.4$; Table~\ref{tab::preservation}), consistent with the dosage reduction attenuating---rather than rewiring---their coexpression.
...
The Leaf Gradient control set, despite carrying substantial coexpression structure (5 modules with stable bootstrap support and large absolute connectivity changes), shows no genotype-specific connectivity loss beyond chance ($\textit{FDR} > 0.2$ for all five modules; Table~\ref{tab::preservation_lg}), indicating that the connectivity weakening described above is specific to the \invfour DEG modules rather than a generic genome-wide attenuation of coexpression in \invfour samples.
...
Chromosomal inversions maintain coadapted allele complexes by suppressing recombination \cite{kirkpatrick2006b, dobzhansky1971}.
...
For the PA2024 hybrid trial, the 41 segregating NIL$_{xxxx}$ lines were treated as a random-effect cohort and the eight named entries (\texttt{B73}, \texttt{Oh43}, \texttt{Inv4\_\{B73, MEX, Mi21, PT\}}, and the two hybrid checks) as fixed effects via \texttt{addCheck = TRUE}; for the PA2022 NIL trial only CTRL and \invfour genotypes are present so both were treated as fixed.
...
\subsection{BZea NIL panel \invfour effect (NC2025)}

To test whether the \invfour flowering time effect generalizes beyond the MI21 single donor background, we analyzed the BZea NIL panel grown in 2025 at the Central Crops Research Station (Clayton, NC).
The BZea panel introgresses chromosomal segments from multiple teosinte donor accessions into the B73 background.
After the both arms BC1 filter (BC1 families containing both \invfour donor and recurrent alleles), the surviving panel comprised four \textit{Z.~mays} subsp.\ \textit{mexicana} ancestry groups (Chalco, Durango, Mesa-Central, Nobogame) and \textit{Z.~mays} subsp.\ \textit{huehuetenangensis} (Huehuetenango); donor lineages that do not carry the \invfour mexicana haplotype (Balsas parviglumis, \textit{Z.~luxurians}, \textit{Z.~diploperennis}) had no donor arm at the \invfour locus and were excluded mechanically by the filter.
We use ``ancestry'' rather than ``race'' for these germplasm groupings to follow current convention in human and increasingly in plant genetics literature where ``race'' is avoided; the column is stored as \texttt{race} in the BZea germplasm metadata and renamed to \texttt{ancestry} on join.
...
Pedigree levels (donor accession, F1, BC1, BC2) were derived as substring prefixes of the NIL identifier; ancestry was joined from the BZea germplasm metadata via donor accession.
...
A pre flight diagnostic showed that intermediate pedigree levels (donor accession, F1, BC1) carry zero additional variance once ancestry (above) and BC2 (below) are accounted for, so we report the parsimonious 3 random effect specification; the full slash nested 6 random effect model gave identical \invfour fixed effect estimates with singular fits at the intermediate levels.
...
Because the directional hypothesis concerns meristem elongation, the SAM Welch \textit{t}-tests in Figure~\ref{fig:phenotypes}D were one tailed (Inv4m $>$ CTRL) and the BH-FDR pool was restricted to the three elongation related dimensions (height, $h/r$, $h/r^2$).
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=\linewidth]{figs/Zeal_Inv4m_flowering_supp.png}
\caption{\textbf{\invfour effect on flowering and plant height in the BZea near isogenic line panel (NC2025).}
...
\begin{table}[!ht]
\caption{\textbf{WGCNA module preservation statistics for the Leaf Gradient topological control set.} Modules sorted by FDR from the genotype-label permutation test used in Fig.~\ref{fig:wgcna}B (1000 shuffles of the CTRL/\invfour labels). $\Delta k_{Within}$ is the mean intramodular connectivity change (\invfour $-$ CTRL); $Z_{\Delta k}$ is that observed value expressed as a $z$-score relative to the permutation null. $Z_{density}$, $Z_{connectivity}$, and $Z_{summary}$ from \texttt{modulePreservation()} with 1000 permutations \cite{langfelder2011}: $Z_{summary} < 2$ = not preserved, $2$--$10$ = moderate, $> 10$ = strong. $Z_{connectivity}$ quantifies rank preservation of intramodular connectivity; $Z_{density}$ quantifies preservation of average connectivity strength. LFP = Largest Fragment Proportion (bootstrap support in CTRL). The Leaf Gradient set groups genes by expression along the leaf developmental gradient and serves as an orthogonal control for the Genotype Response modules in Table~\ref{tab::preservation}.}
