...
We found that field expression of the cluster fell significantly below copy number expectation, indicating additional regulatory suppression.
...
Before its expansion into temperate regions, humans introduced maize to the Mexican highlands and the Southwestern United States, where sympatry with highland teosinte likely facilitated the introgression of adaptive alleles from \mex.
...
\subsection{The \invfour introgression alters flowering time and plant height, with responses that vary by environment}
...
Given prior evidence that \invfour promotes growth, we applied one-tailed $t$-tests (H$_0$: $\mu_{\text{Inv4m}} \leq \mu_{\text{CTRL}}$).
...
Our transcriptomic analysis identifies the JUMONJI histone demethylase cluster as the strongest candidate: the apparent downregulation largely reflects a lineage-specific tandem expansion in B73 (five paralogs versus a single ortholog in highland genomes; Fig~\ref{fig:jmj_cluster}B), but field expression falls significantly below the copy number expectation, indicating additional regulatory suppression (Fig~\ref{fig:jmj_cluster}A).
WGCNA shows that the \invfour introgression disrupts 12 of 13 coexpression modules (Table~\ref{tab::preservation}), with JMJ genes losing hub status (Fig~\ref{fig:wgcna}C).
We tested and rejected the phosphorus starvation hypothesis in our companion study~\cite{rodriguez2026phosphorus}; here we focus on identifying candidate genes underlying the observed phenotypes.

\subsection*{Breakpoint characterization implicates knob repeats in inversion structure}

We delimited \invfour to 15.2 Mb containing 432 annotated genes in B73 through genome alignments (Table~\ref{tab:breakpoints}).
We found the breakpoint segments enriched for chromosomal knob repeats: the upstream breakpoint contains 438 \textit{knob180} and 29 \textit{TR-1} repeats (74\% of annotated repeats), while the downstream breakpoint contains 37 \textit{knob180} repeats (46\%; Fig~\ref{fig::design}C).
...
Internode measurements in the NC2025 field trial localized the height effect: the largest difference was concentrated at internode position 2, just below the tassel ($-4.1$~cm, $p < 10^{-19}$; Supplementary Fig.~\ref{fig::internode}), while total node count did not differ between genotypes.
...
Analysis across three location-year combinations identified significant GxE interactions for all phenotypes tested (Supplementary Table~\ref{tab::gxe}).
...
Microsynteny analysis showed that the B73 reference carries a lineage-specific tandem expansion of five JMJ paralogs, while highland teosinte TIL18, the highland landrace PT, and the CIMMYT tropical inbred lines CML457 and CML459 each retain only a single \textit{jmj6} ortholog (Fig~\ref{fig:jmj_cluster}B).
...
%
% =============================================================================
% BibTeX entries to add to Inv4m.bib (SAM-flowering references)
% =============================================================================
%
% @article{leiboff2015,
...
%   author  = {Thompson, Addie M and Yu, Jianming and Timmermans, Marja C P and Schnable, Patrick and Crants, Johanna C and Scanlon, Michael J and Muehlbauer, Gary J},
...
%
% @article{ku2008,
%   author  = {Ku, L and Huang, Q},
%   title   = {Determination of the photoperiod-sensitive inductive phase in maize with leaf numbers and morphologies of stem apical meristem},
%   journal = {Agricultural Sciences in China},
%   year    = {2008},
%   volume  = {7},
%   number  = {5},
%   pages   = {554--560},
%   doi     = {10.1016/S1671-2927(08)60053-0}
% }
%
% @article{danilevskaya2008,
%   author  = {Danilevskaya, Olga N and Meng, Xin and Selinger, Dave A and Deschamps, Stéphane and Hermon, Pedro and Vansant, Gina and Gupta, Rajeev and Ananiev, Evgueni V and Muszynski, Michael G},
%   title   = {Involvement of the {MADS}-box gene {ZMM4} in floral induction and inflorescence development in maize},
...
%
% @article{wu2016,
%   author  = {Wu, Lei and Wang, Xin and Wang, Shuang and Wu, Lei and Tian, Lei and Tian, Zhixi and Liu, Peng and Jiang, Hong},
%   title   = {Comparative proteomic analysis of the shoot apical meristem in maize between a {ZmCCT}-associated near-isogenic line and its recurrent parent},
%   journal = {Scientific Reports},
%   year    = {2016},
%   volume  = {6},
%   pages   = {30641},
%   doi     = {10.1038/srep30641}
% }
