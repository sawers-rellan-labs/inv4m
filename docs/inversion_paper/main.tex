...
To test whether the absolute connectivity loss is specifically driven by \invfour genotype rather than by sample-to-sample variability, we complemented the within-module $t$-test on $\Delta k_{Within}$ (Table~\ref{tab::preservation}) with a genotype-label permutation test (Fig.~\ref{fig:wgcna}B): for each module, we computed the null distribution of $\Delta k_{Within}$ under 1000 random shuffles of the CTRL/\invfour labels and FDR-corrected the observed value against this null.
Five of the 13 Genotype Response modules (brown, tan, purple, blue, pink; Fig.~\ref{fig:wgcna}B, top facet) reached significance under the permutation test ($\textit{FDR} < 0.05$), confirming that their connectivity losses are specifically associated with the \invfour introgression and not with random sampling variation.
...
The 5 Leaf Gradient modules (sky, peach, jade, coral, lavender) all reached stable bootstrap support (LFP $\geq 0.81$; Fig.~\ref{fig::module_support}), and all showed large, significant absolute $\Delta k_{Within}$ losses in the $t$-test (from $-1.44$ to $-15.20$; $\textit{FDR} < 10^{-3}$; Table~\ref{tab::preservation_lg}).
Under the genotype-label permutation test, however, \textit{none} of the 5 Leaf Gradient modules reached significance (all FDR $> 0.2$, Fig.~\ref{fig:wgcna}B, bottom facet): the permutation null absorbs the absolute connectivity change, because the Leaf Gradient hubs were selected to vary with leaf stage rather than with genotype.
This orthogonal control thus isolates a genotype-specific connectivity weakening restricted to a subset of \invfour DEG modules, rather than a genome-wide attenuation of coexpression.
...
\begin{table}[!ht]
\caption{\textbf{WGCNA module preservation statistics for the Genotype Response (Inv4m DEG) reference network.} Modules sorted by FDR-corrected $p$-value from one-sample $t$-test of $\Delta k_{Within} \neq 0$. $Z_{density}$, $Z_{connectivity}$, and $Z_{summary}$ from \texttt{modulePreservation()} with 1000 permutations \cite{langfelder2011}: $Z_{summary} < 2$ = not preserved, $2$--$10$ = moderate, $> 10$ = strong. $Z_{connectivity}$ quantifies rank preservation of intramodular connectivity; $Z_{density}$ quantifies preservation of average connectivity strength. LFP = Largest Fragment Proportion (bootstrap support in CTRL).}
\label{tab::preservation}
\centering
\small
\begin{tabular}{@{}lrrrrrrr@{}}
\toprule
\textbf{Module} & \textbf{Genes} & \textbf{$Z_{density}$} & \textbf{$Z_{connectivity}$} & \textbf{$Z_{summary}$} & \textbf{$\Delta k_{Within}$} & \textbf{FDR} & \textbf{LFP} \\
\midrule
blue & 51 & 1.9 & 4.4 & 3.1 & -1.27 & $4.9 \times 10^{-10}$ & 0.79 \\
brown & 46 & 3.9 & 3.8 & 3.8 & -2.22 & $4.3 \times 10^{-9}$ & 0.68 \\
yellow & 36 & 2.6 & 4.4 & 3.5 & -1.29 & $1.5 \times 10^{-8}$ & 0.68 \\
purple & 22 & 4.4 & 3.3 & 3.8 & -1.04 & $2.6 \times 10^{-6}$ & 0.74 \\
black & 32 & 5.1 & 3.9 & 4.5 & -0.66 & $2.6 \times 10^{-6}$ & 0.69 \\
pink & 30 & 5.1 & 6.4 & 5.8 & -0.83 & $2.6 \times 10^{-6}$ & 0.72 \\
red & 34 & 4.4 & 2.2 & 3.3 & -0.34 & $8.5 \times 10^{-5}$ & 0.63 \\
greenyellow & 16 & 3.5 & 2.4 & 3.0 & -0.65 & $4.5 \times 10^{-4}$ & 0.80 \\
green & 36 & 7.8 & 2.2 & 5.0 & -0.38 & 0.002 & 0.69 \\
tan & 12 & 2.5 & 2.0 & 2.2 & -4.13 & 0.003 & 0.68 \\
turquoise & 55 & 6.5 & 5.6 & 6.1 & -0.33 & 0.005 & 0.54 \\
salmon & 11 & 0.7 & 1.7 & 1.2 & -0.15 & 0.025 & 0.52 \\
magenta & 24 & 7.7 & 2.9 & 5.3 & -0.18 & 0.126 & 0.71 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!ht]
\caption{\textbf{WGCNA module preservation statistics for the Leaf Gradient topological control set.} Modules sorted by FDR-corrected $p$-value from one-sample $t$-test of $\Delta k_{Within} \neq 0$. $Z_{density}$, $Z_{connectivity}$, and $Z_{summary}$ from \texttt{modulePreservation()} with 1000 permutations \cite{langfelder2011}: $Z_{summary} < 2$ = not preserved, $2$--$10$ = moderate, $> 10$ = strong. $Z_{connectivity}$ quantifies rank preservation of intramodular connectivity; $Z_{density}$ quantifies preservation of average connectivity strength. LFP = Largest Fragment Proportion (bootstrap support in CTRL). The Leaf Gradient set groups genes by expression along the leaf developmental gradient and serves as an orthogonal control for the Genotype Response modules in Table~\ref{tab::preservation}. Note: the $t$-test FDR reported here tests whether within-module $\Delta k_{Within}$ is consistently below zero but does not isolate a genotype-specific effect. The genotype-label permutation test in Fig.~\ref{fig:wgcna}B is the relevant test for genotype specificity: under that test, none of the Leaf Gradient modules reach significance ($\textit{FDR} > 0.2$ for all 5), while 5 of 13 Genotype Response modules do.}
...
\textbf{Module} & \textbf{Genes} & \textbf{$Z_{density}$} & \textbf{$Z_{connectivity}$} & \textbf{$Z_{summary}$} & \textbf{$\Delta k_{Within}$} & \textbf{FDR} & \textbf{LFP} \\
\midrule
sky & 78 & 5.1 & 10.7 & 7.9 & -15.20 & $1.4 \times 10^{-31}$ & 0.83 \\
jade & 30 & 2.5 & 6.2 & 4.4 & -4.80 & $2.5 \times 10^{-10}$ & 0.95 \\
coral & 26 & 3.8 & 4.8 & 4.3 & -2.59 & $2.4 \times 10^{-6}$ & 0.95 \\
lavender & 13 & 3.6 & 4.5 & 4.1 & -1.44 & $8.7 \times 10^{-5}$ & 0.81 \\
peach & 64 & 8.4 & 8.3 & 8.3 & -2.18 & $8.8 \times 10^{-4}$ & 0.86 \\
\bottomrule
\end{tabular}
\end{table}
