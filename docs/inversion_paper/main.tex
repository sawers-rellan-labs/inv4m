...
\centering
\includegraphics[width=\linewidth]{figs/design.png}
\caption[\textit{\invfour} Delimitation, Introgression Breeding Design and Population Genotypes.]
{\textit{\textbf{Chromosomal Inversion \invfour: Delimitation,  Introgression Breeding Design, Field Experimental Setup and Population Genotype.}} }
\label{fig::design}
\end{figure*}
...
\textbf{(C)} PT shows higher (** \textit{t-test} $p=0.0022$) leaf phosphorus content than B73 under phosphorus sufficiency (Puerto Vallarta, 2016). 
...
\textbf{(E)}  Rock Springs, PA, field station experimental setup at 5 ppm (-P), and  36 ppm (+P) soil phosphorus [mg/kg Mehlich-3].
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=0.95\linewidth]{figs/RNAseq.png}
...
\textbf{(B)} Multidimensional Scaling Analysis of the leaf transcriptome. Samples are colored by genotype (CTRL: yellow; \invfour: purple) and shaped by leaf stage, showing clear separation along dimension 3 (8\% variance). 
...
\textbf{(F)} Haplotype map of the introgressed region on chromosome 4 for representative NIL lines (4125, 4131, 3131). The blue triangle indicates the tagging SNP PZE04175660223 used for selection. 
\textbf{(G)} Local Manhattan plot zoomed into the 150-200 Mb region of chromosome 4, highlighting the clustering of \fdrls markers within the \invfour boundaries. 
\textbf{(H)} Local distribution of $\log_2$(Fold change) across chromosome 4. Moving average lines (green for upregulated, red for downregulated) illustrate the coordinated transcriptomic shift within the \invfour region, specifically the downregulation of the \jmjii cluster.
}
...
\cite{perez-valencia2022}. For each phenotype $y$, SpATS fits a mixed model
