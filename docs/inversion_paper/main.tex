...
To assess whether the \invfour introgression causes broader network-level
perturbations beyond individual DEGs, we performed weighted gene coexpression
network analysis (WGCNA) using a consensus approach with 1000 bootstrap
iterations (Fig~\ref{fig:wgcna}A). We constructed reference networks using
CTRL samples and evaluated how module structure and connectivity changed in
\invfour plants.

The consensus network identified 13 coexpression modules containing 465 DEGs
(FDR $< 0.05$). Comparison of intramodular connectivity ($k_{Within}$) between
genotypes revealed systematic connectivity loss in \invfour samples
(Fig~\ref{fig:wgcna}B). Most modules showed substantial decreases in
$\Delta k_{Within}$, with the blue, yellow, and brown modules exhibiting the
strongest disruption. This pattern indicates that genes normally coexpressed
in wild-type plants lose their coordinated expression in \invfour, consistent
with regulatory perturbation emanating from the introgressed region.

Hub gene analysis revealed dramatic connectivity changes for key regulators
(Fig~\ref{fig:wgcna}C). In CTRL samples, \jmjii was a highly connected hub
gene with $k_{Within} = 19.4$; in \invfour samples, its connectivity collapsed
to $k_{Within} = 0.33$, representing a 98\% reduction. Similarly, \jmjiv
showed $k_{Within} = 15.4$ in CTRL versus $0.16$ in \invfour (99\% reduction).
These JMJ genes transitioned from central network hubs to peripheral,
disconnected nodes, suggesting that their downregulation disrupts the
coexpression relationships they normally coordinate.

In contrast, some upregulated genes showed increased connectivity in \invfour.
The exocyst component \textit{sec6} ($\log_2 FC = +2.82$) maintained moderate
connectivity ($k_{Within} = 5.5$ in CTRL, $0.54$ in \invfour), while genes in
strongly upregulated modules gained new coexpression relationships. This
bidirectional rewiring---loss of connectivity for downregulated chromatin
regulators and gain for upregulated genes---is consistent with the JMJ cluster
functioning as a master regulatory hub whose perturbation propagates through
the transcriptome.
...
\textbf{(B)} Change in intramodular connectivity ($\Delta k_{Within}$) between CTRL and \invfour genotypes. Each box represents a module, colored by module assignment. Negative values indicate connectivity loss in \invfour. Most modules show substantial connectivity decrease, with blue, yellow, and brown modules exhibiting the strongest disruption.
...
\begin{table}[!ht]
\caption{\textbf{WGCNA module preservation statistics.} Summary of consensus module preservation between CTRL and \invfour genotypes. Adjacency correlation measures the similarity of within-module connectivity patterns between genotypes. $\Delta k_{Within}$ indicates median change in intramodular connectivity (negative = loss). Bootstrap support (LFP) indicates module stability in CTRL samples across 1000 iterations. All modules show reduced connectivity in \invfour, with adjacency correlations ranging from $-0.14$ to $0.42$.}
\label{tab::preservation}
\centering
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
Module & Genes & Adj. Corr. & $\Delta k_{Within}$ & Bootstrap Support \\
\midrule
blue & 51 & 0.01 & $-0.71$ & 0.79 \\
yellow & 36 & 0.06 & $-0.59$ & 0.68 \\
brown & 46 & 0.19 & $-0.59$ & 0.68 \\
pink & 30 & 0.02 & $-0.54$ & 0.72 \\
tan & 12 & 0.41 & $-0.54$ & 0.68 \\
greenyellow & 16 & 0.28 & $-0.51$ & 0.80 \\
black & 32 & 0.33 & $-0.47$ & 0.69 \\
purple & 22 & $-0.14$ & $-0.43$ & 0.74 \\
green & 36 & 0.14 & $-0.39$ & 0.69 \\
red & 34 & 0.16 & $-0.32$ & 0.63 \\
turquoise & 55 & 0.42 & $-0.19$ & 0.54 \\
magenta & 24 & 0.07 & $-0.08$ & 0.71 \\
salmon & 11 & $-0.03$ & 0.01 & 0.52 \\
\bottomrule
\end{tabular}
\end{table}
