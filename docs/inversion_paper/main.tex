...
The single-copy state is shared across highland and tropical lines, indicating it is likely the ancestral configuration, with the five-copy expansion specific to the B73 lineage.
...
We therefore asked whether \invfour expression deviates from what copy number alone would predict (CTRL/5).
...
GWAS independently links the JMJ cluster to flowering time variation \cite{tibbs-cortes2024}.
JUMONJI domain proteins function as histone demethylases that remove methyl groups from H3K4, a mark associated with active transcription \cite{klose2007}, and in Arabidopsis, JMJ homologs regulate flowering time through chromatin modification of floral pathway genes \cite{gan2014}.
Crow et al.\ \cite{crow2020} identified JMJ demethylases among their top candidates but lacked the genome assemblies needed to characterize the underlying copy number variation.
Our microsynteny analysis provides this structural explanation and, together with the environment-dependent suppression and GWAS support, positions the JMJ cluster as a candidate not only for differential expression but for the environment-dependent flowering and growth phenotypes we observed.
