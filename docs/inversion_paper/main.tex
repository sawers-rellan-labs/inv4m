...
A linear mixed model with strictly nested random effects (ancestry / BC2 family / NIL; see Methods) recovered the flowering time signal we observed in the MI21 single donor panel: plants carrying the donor \invfour inverted karyotype silked $0.88 \pm 0.31$ days earlier ($p = 0.004$, FDR $= 0.013$) and reached anthesis $0.61 \pm 0.27$ days earlier ($p = 0.022$, FDR $= 0.033$) than their recurrent (B73 standard karyotype) siblings within the same NIL line, with within BC2 permutation $p$ values agreeing within rounding (Supplementary Fig.~\ref{fig::zeal_inv4m}A,B).
...
Notably, the Reference network also recovered edges involving multiple genes with documented plant architecture phenotypes in other plant systems.
The histone demethylase \jmjii (\textit{Zm00001eb191790}; $\log_2 FC = -3.49$) and its paralog \textit{jmj21} (\textit{Zm00001eb190750}; $\log_2 FC = +2.28$) connect to trans targets, consistent with the flowering time and developmental phenotypes documented for JmjC histone demethylase orthologs in Arabidopsis \cite{gan2014} and rice \cite{song2018}.
\textit{smo4} (\textit{Zm00001eb192850}; $\log_2 FC = -1.98$), encoding a Ribosome biogenesis protein NOP53 homolog, also appears in the Reference network; its Arabidopsis ortholog \textit{SMALL ORGAN 4} (\textit{SMO4}) regulates cell proliferation rate during organ growth, and \textit{smo4} mutants exhibit reduced organ size \cite{zhang2015b}.
A fourth Reference-network gene, \textit{borr} (\textit{Zm00001eb192330}; $\log_2 FC = +5.05$), is the maize ortholog of Arabidopsis AT4G39630 (\textit{BORR}, Borealin-related) and carries a Borealin Pfam domain (PF10512), encoding a chromosomal passenger complex kinetochore component involved in cell division.
...
To test whether the \invfour flowering time effect generalizes beyond the MI21 single donor background, we analyzed the ZEAL NIL panel grown in 2025 at the Central Crops Research Station (Clayton, NC).
...
\textbf{(A)} Lineage corrected days to anthesis (DTA, left) and days to silking (DTS, right) for CTRL (recurrent B73 standard karyotype at \invfour, gold) and \invfour (donor mexicana or huehuetenangensis inverted karyotype, purple).
...
Donor lineages that do not carry the \invfour inverted karyotype (parviglumis, \textit{Z.~luxurians}, \textit{Z.~diploperennis}) are excluded because their BC1 families are monomorphic standard at \invfour (no donor arm to estimate a within family contrast).}
\label{fig::zeal_inv4m}
\end{figure*}


\begin{table}[!ht]
\caption{\textbf{Effect of \invfour on gene expression.} Selected DEGs from the plant-blocking limma model (FDR $< 0.05$, $|\log_2 \text{FC}| > 1.5$). Bottom rows show significant Leaf$\times$\invfour interactions. The \textbf{Label} column is an editorial shorthand used throughout the manuscript and network plots; it corresponds to the MaizeGDB locus symbol when that symbol is informative, and otherwise to a curated mnemonic that does not imply naming orthodoxy.}
...
Zm00001eb192850 & \textit{smo4} & 17.5 & $-1.98$ & Ribosome biogenesis protein NOP53 \\
...
Zm00001eb112470 & \textit{engd2} & 12.3 & $+6.14$ & OBG-type G domain-containing protein \\
...
\textbf{(B)} GO enrichment of the \invfour trans coexpression network from Fig.~\ref{fig:wgcna}C, stratified by gene location (flanking introgression vs.\ \invfour proper) and regulation direction.
Notable enrichments include negative regulation of gene expression (epigenetic) and positive regulation of growth among \invfour-downregulated genes.
...
Each point shows one of the top five hub genes (by CTRL $k_{Within}$) in each WGCNA consensus module; point color corresponds to module identity as in Fig.~\ref{fig:wgcna}.
...
This figure complements Fig.~\ref{fig:wgcna}B, which summarizes module-level connectivity loss as a permutation-based $z$-score.}
...
Minimum spanning tree of all dataset-specific co-expression edges not present in the MaizeNetome reference, filtered to Bonferroni-adjusted $p < 0.001$ across the cis $\times$ trans correlation test pool ($m \approx 1{,}095$ tests, $n = 43$ samples).
...
This figure shows the complete novel network; the \jmjiv neighborhood subgraph is detailed in Fig.~\ref{fig:wgcna}C.}
...
\midrule
  ...
  \textit{Zm00001eb193130} & \textit{chi} & Chalcone--flavonone isomerase & -5.47 & 2.61e-07 \\
  ...
  \textit{Zm00001eb194180} & --- & troponin T, skeletal muscle-like & 3.97 & 4.68e-06 \\
...
\midrule
  ...
  \textit{Zm00001eb000540} & \textit{prd4} & Phosphatidylinositol N-acetyglucosaminlytransferase subunit P-related & 2.32 & 3.99e-10 \\
  \textit{Zm00001eb025970} & \textit{ppp} & Serine/threonine protein phosphatase & 5.24 & 1.87e-09 \\
  ...
  \textit{Zm00001eb332450} & \textit{roc4-2} & Peptidyl-prolyl cis-trans isomerase & -2.92 & 1.69e-05 \\
  ...
  \textit{Zm00001eb213980} & \textit{dwy1} & Pentatricopeptide repeat-containing protein & -5.21 & 9.70e-03 \\
...
Zm00001eb112470 & \textit{engd2} & OBG-type G domain-containing protein & $+6.14$ & $4.9 \times 10^{-13}$ \\
