...
\begin{figure*}[!ht]
\includegraphics[width=0.6\linewidth]{figs/effects.png}
\centering
\caption{\textbf{Effect of \invfour on field phenotypes and shoot apical meristem architecture.}
CTRL (yellow) vs \invfour (purple) per-plot spatially-corrected phenotypes; stage-2 contrasts described in Methods.
\textbf{(A) NILs.} PA2022 NIL field trial: plant height (PH, cm), days to anthesis (DTA, days), and days to silking (DTS, days).
\textbf{(B) Hybrids.} PA2024 NIL-derived hybrid field trial focal contrast \texttt{Inv4\_Mi21} vs \texttt{Inv4\_B73}: plant height (PH, cm), total grain weight (TGW, g), and total grain number (TGN).
\textbf{(C) 21 DAP Seedling SAMs.} Representative differential interference contrast micrographs of cleared vegetative shoot apical meristems; yellow lines indicate height ($h$) and radius ($r$). Scale bars: 100 $\mu$m.
\textbf{(D) SAMs.} SAM shape descriptors derived from C: height-to-radius ratio ($h/r$) and shape factor ($h/r^2$, nm$^{-1}$), two-sided Welch \textit{t}-tests.
...
To investigate whether these macroscopic phenotypes were associated with changes in meristem organization, we examined shoot apical meristems (SAMs) from 21-day-after-planting seedlings grown in controlled environment chambers (Fig~\ref{fig:phenotypes}C,D).
...
SAM measurements from 21-day-old seedlings grown in controlled environment chambers showed that \invfour plants have elongated meristems, with a 9\% increase in SAM height ($+9.65 \pm 5.50$ $\mu$m, $p = 0.044$, one-tailed) and significant elevation of the height-to-radius ratio and shape factor ($h/r^2$) under two-sided Welch \textit{t}-tests (Fig~\ref{fig:phenotypes}C,D).
