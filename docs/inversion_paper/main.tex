...
To obtain robust module assignments, we employed a consensus network approach with
1000 iterations. In each iteration, 80\% of genes were randomly sampled and
hierarchical clustering was performed on the topological overlap matrix (TOM) with
\textit{minModuleSize} uniformly sampled from 10--35 to avoid parameterization bias.
A co-clustering matrix was constructed by counting how often gene pairs were assigned
to the same module across iterations. The final consensus adjacency was computed as
the ratio of co-clustering counts to co-sampling counts, and modules were identified
by cutting the resulting dendrogram (\textit{deepSplit} = 4).
