...
Comparative genomics across five \textit{Zea} genomes revealed that the B73 reference carries a lineage specific tandem expansion of five JMJ paralogs, while highland teosinte, a highland landrace, and CIMMYT tropical inbreds each retain a single ancestral copy.
So at this locus, it is the temperate reference, not the highland inverted allele, that carries the derived configuration.
...
Coexpression analysis showed that the introgression specifically disrupts a subset of growth related coexpression modules, with the \jmjii/\jmjiv cluster losing hub connectivity, and a \textit{trans} regulatory network connects the cluster to cell proliferation genes including \textit{pcna2} and \textit{sec6}.
Together, these results identify candidate genes and regulatory networks that might underlie \invfour's contribution to highland adaptation.
\end{abstract}
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\lettrine[lines=2]{\color{color2}C}{hromosomal} inversions are a widespread form of structural variation, segregating in natural populations of plants, animals, fungi, and humans \citep{Wellenreuther2018-an, kirkpatrick2006, kirkpatrick2010}.
By suppressing recombination between standard and inverted haplotypes, inversions can maintain coadapted allele complexes as single inherited units, a mechanism originally proposed by
...
Highland-associated chromosomal inversions, such as \invfour and \textit{Inv9f}, among others, are prevalent in highland teosinte populations \citep{pyhajarvi2013} and traditional Mexican highland maize varieties (TVs) \citep{crow2020,gonzalez-segovia2019-jy} but are rare in temperate maize.
...
First we used long read assemblies of highland materials and the Mi21 NIL to precisely delimit \invfour breakpoints.
We found that \invfour breakpoints overlap chromosomal knob repeat arrays consistent with an ectopic recombination origin.
...
\invfour modulated flowering time and plant height across our field environments, with genotype by environment interactions congruent with antagonistic selection, and produced elongated shoot apical meristems in seedlings, a developmental signature of accelerated flowering \citep{danilevskaya2008, wu2008, leiboff2015, thompson2015}.
We then traced the most significant gene expression difference to a cluster of JUMONJI histone demethylases (JMJ).
At the \invfour locus, microsynteny across teosinte \textit{mexicana}, three highland maize references (the landrace Palomero Toluqueño and the CIMMYT highland inbreds CML457 and CML459), and the Mi21 NIL carrying the highland \invfour introgression showed a single JMJ copy in every case, while the B73 reference carries a tandem expansion of five paralogs (Fig.~\ref{fig:jmj_cluster}~B).
The single copy state is shared across teosinte lineages, while the multicopy state predominates in cultivated maize, where most assemblies in MaizeGDB show expansion relative to the teosinte configuration (Supplementary Fig.~\ref{fig::jmj_teosinte}).
...
Co-expression analysis revealed that the introgression rewires native B73 networks: the \jmjii/\jmjiv cluster loses hub connectivity in growth related coexpression modules, and a \textit{trans}-regulatory network connects the cluster to cell proliferation genes including \textit{pcna2} and \textit{sec6}, pointing to a working hypothesis in which JMJ dosage modulates growth and flowering time through chromatin modification.
...
The dashed rectangle on the B73 panel marks the estimated introgression extent ($\sim$141--170~Mb in Mi21 NIL; $\sim$158--191~Mb in B73 coordinates), determined from positional offset analysis of the alignment.
...
To investigate whether these whole plant phenotypes were associated with changes in meristem organization, we examined shoot apical meristems (SAMs) from two week old seedlings grown in controlled environment chambers (Fig~\ref{fig:phenotypes}B,C).
...
These genes showed the strongest significance in \invfour plants, with \jmjii exhibiting a $\log_2 \text{FC} = -3.49$ ($\textit{FDR} < 10^{-20}$; Supplementary Table~\ref{tab:gene_effects}).
...
\jmjii is among the pink module's top hubs; both \jmjii and \jmjiv had high $k_{Within}$ in CTRL ($\sim$3.3 and $\sim$3.2 respectively) and dropped to $\sim$0.6 in \invfour (Fig~\ref{fig:networks}C). 
...
%Gene Ontology enrichment analysis of genes within \invfour proper (Fig~\ref{fig::transnetwork}B) found significant overrepresentation of negative regulation of gene expression, epigenetic (GO:0045814; $p = 2.2 \times 10^{-4}$, fold enrichment = 90.4) and positive regulation of growth (GO:0045927; $p = 1.3 \times 10^{-3}$, fold enrichment = 37.7).
...
Field expression of the cluster falls below the dosage expectation predicted by copy number alone, the \jmjii/\jmjiv cluster loses hub connectivity in growth related coexpression modules, and a \textit{trans}-regulatory network links the cluster to cell proliferation genes including \textit{pcna2} and \textit{sec6}.
...
From top to bottom: the teosinte TIL18 (\textit{Z. mays} ssp. \textit{mexicana}), the highland landrace Palomero Toluque\~{n}o (PT), and the CIMMYT highland inbreds CML457 and CML459 each contain a single \textit{jmj6} ortholog, while B73 (bottom) has an expanded tandem array of five paralogs (\textit{jmj9}, $\psi$?, \textit{jmj6}, \jmjii, \jmjiv).
...
Microsynteny across five \textit{Zea} genomes places the expansion of this cluster in the temperate B73 lineage rather than the highland inverted allele (Fig~\ref{fig:jmj_cluster}B).
B73 carries five paralogs, while highland teosinte (TIL18), the highland landrace Palomero Toluqueño, and the CIMMYT highland inbreds CML457 and CML459 each retain a single \textit{jmj6} ortholog.
...
% PASS2-CUT (user flagged 2026-05-26 as car-salesman prose): "thus emerges as a recurring axis" + general framing. Replacement TBD by user.
...
\jmjii and \jmjiv sit in the pink module, which is among the five modules with significant module-level connectivity loss in the permutation test; both genes had $k_{Within} \sim 3.2$ in CTRL and $\sim 0.6$ in \invfour.
...
%This pattern of effect reversal across latitudes parallels the elevation dependent fitness tradeoffs reported for \invfour in Mexican landrace field trials \cite{crow2020} and is consistent with the antagonistic selection discussed above.
...
A related but distinct caveat applies to our differential expression analysis, which preceded the availability of the Mi21 NIL assembly reported here and used the B73 reference for read mapping.
Sequence divergence between B73 and Mi21 inside \invfour (median CDS divergence $\sim$1.1\%) raises the possibility that some apparent expression differences at the most diverged genes reflect reduced read mapping for diverged transcripts rather than regulatory change.
...
To obtain a reference-quality genome for the \invfour donor parent haplotype, we performed \textit{de novo} assembly of the Mi21 near isogenic line from PacBio HiFi sequencing data.
...
To examine microsynteny at the JMJ cluster, we performed pairwise ortholog detection between B73 and the Mi21 NIL assembly using jcvi/MCScan \cite{tang2008jcvi} with LAST \cite{kielbasa2011} alignments (\texttt{--cscore=.99}, \texttt{--minspan=30}).
...
CDS sequences for B73 (NAM v5) and the Mi21 NIL were aligned using LAST \cite{kielbasa2011} as part of the microsynteny pipeline described above.
For each B73 gene in the shared introgression with a reciprocal best hit in Mi21 NIL, we retained the highest scoring alignment and computed CDS divergence as $100 - \text{\% identity}$.
...
Following \cite{shahan2018}, the consensus coclustering matrix $C$ was computed as:
\begin{equation}
