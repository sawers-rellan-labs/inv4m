...
To test whether this pattern is specific to the \invfour DEG set or reflects a genome-wide attenuation of coexpression in the \invfour transcriptome, we built an orthogonal ``Leaf Gradient'' reference network from genes selected by their expression dynamics along the leaf developmental gradient rather than by differential expression between genotypes (Methods).
The 5 Leaf Gradient modules (sky, peach, jade, coral, lavender) all reached stable bootstrap support (LFP $\geq 0.81$; Fig.~\ref{fig::module_support}) and all showed significant connectivity loss in \invfour (FDR $< 10^{-3}$; Table~\ref{tab::preservation_lg}), mirroring the Genotype Response pattern of preserved rank structure ($Z_{connectivity}$ 4.5 to 10.7) alongside substantial absolute reduction (mean $\Delta k_{Within}$ from $-1.44$ to $-15.20$).
This orthogonal control indicates that the rank-preserving, magnitude-attenuating pattern we observed in the \invfour DEG network extends to unrelated gene modules, consistent with a genome-wide weakening of coexpression rather than a DEG-specific artifact.
