...
\subsection*{Leaf stage cause global changes in gene expression, while \invfour introgression drives local transcriptional effects.}

Our analysis of leaf transcriptome data revealed distinct patterns of gene expression driven by developmental stage, and \invfour introgression.
The third dimension of expressionn MDS, with 8\% of explained variance, is associated with \invfour, separating samples by  genotype ($r=0.92$, $\textit{FDR} = 2.06\times 10^{-17}$).
Out of 24011  genes detected in at least one leaf sample, we found 8159 differential expressed genes responding to leaf stage,In contrast, significant transcriptional responses to the \invfour introgression were predominantly localized to the inversion and its flanking regions (Fig~\ref{fig::RNAseq} F).
To characterize the distribution of \invfour responsive genes, we classified differentially expressed genes (DEGs) according to their physical and genetic linkage to the inversion.
We defined \textit{trans} regulated genes as those outside the inversion and its surrounding linkage drag, while \textit{cis} regulated genes were those within the 39 Mb shared introgression spanning the inversion.
The distribution of DEGs responsive to \invfour introgression in the genome by location relative to the \invfour locus is presented in Table~\ref{tab:DEGs_distro}.
Among the 970 DEGs responding to \invfour introgression, 646 were \textit{trans} regulated while 324 were regulated in \textit{cis}.
The \textit{cis} regulated genes were proportionally distributed  (Fisher's exact test $p=0.28$) between the flanking regions (183 genes in 24 Mb) and \invfour proper (141 genes in 15 Mb).
DEGs responding to \invfour were 40 times more likely to originate from the shared introgression (Fisher's exact test $p=2.38 \times 10^{-300}$) and 45 times more likely to come from the inversion proper compared to \textit{trans} regulated genes (Fisher's exact test $p=2.73 \times 10^{-142}$).
This locus enrichment was even more pronounced among DEGs with high magnitude response, $log_2[\text{FC}]>2$, hereafter named top DEGs.
Of 139 top DEGs, 96 were located in \textit{cis} (61 in flanking regions, 35 in \invfour proper) and 43 in \textit{trans}, representing 70\% and 35\% of top DEGs in the shared introgression and inversion proper, respectively. 
These top DEGs were 102 times more likely to originate from the shared introgression, 112 times more likely from the flanking segments, and 87 times more likely from the \invfour proper compared to \textit{trans} regulated genes (Fisher's exact test $p < 1 \times 10^{-48}$).
The peak of significance for differential expression due to \invfour introgression is located inside \invfour proper and corresponds to the cluster of \textit{JUMONJI} DNA demethylases that includes \jmjii, \jmjvi and \jmjiv (Fig~\ref{fig::RNAseq} F to I).
However, the distributions of \textit{p-values} for differential expression due to \invfour do not differ between \invfour proper and flanking regions, while both regions have excess significance relative to the rest of the genome (Kolmogorov–Smirnov test $p=0.29$ and $p\leq 2.26 \times 10^{-16}$ respectively, Supplementary Figure S ).
Although a majority of the DEGs in \invfour proper are downregulated (82 genes, 58\%), the proportion of repressed genes is not different to that in the flanking the regions (102 genes, 56\%) (Fisher's exact test $p=0.73$) with a similar result for top DEGs (Fisher's exact test $p=0.39$).
...
\subsection{WGCNA shows \invfour perturpation of existing coexpression modules in
B73 plants}
...
\subsection{Trans-regulatory network analysis reveals epigenetic regulators
connecting \invfour to genome-wide expression changes}
...
\begin{table}[!ht]
\caption{\textbf{WGCNA module preservation statistics.} Summary of consensus module preservation between CTRL and \invfour genotypes. Adjacency correlation measures the similarity of within-module connectivity patterns between genotypes. Bootstrap support (LFP) indicates module stability in CTRL samples across 1000 iterations. Preservation status: Disrupted ($r < 0.3$), Weak ($0.3 \leq r < 0.5$). All modules show substantial disruption in \invfour, with adjacency correlations ranging from $-0.14$ (purple) to $0.42$ (turquoise).}
\label{tab::preservation}
\centering
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
Module & Genes & Adj. Corr. & $\Delta k_{Within}$ & Bootstrap Support & Status \\
\midrule
\textit{[Table to be generated from preservation\_summary.csv]} & & & & & \\
\bottomrule
\end{tabular}
\end{table}
