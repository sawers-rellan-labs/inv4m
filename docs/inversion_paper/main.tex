...
%%% Abstract %%%%%%%%%%%%%%%%%%
\begin{abstract}
Chromosomal inversions can facilitate local adaptation by protecting coadapted allele complexes from recombination.
\invfour is a 15 Mb inversion from the highland teosinte \textit{Zea mays} ssp. \mex that is nearly fixed above 2500 m in Mexico but rare in temperate maize.
We introgressed \invfour into the B73 background through eight generations of backcrossing and characterized the effects of the introgression on phenotype, transcriptome, and gene coexpression networks.
Genome alignment to three \textit{Zea} references delimited \invfour breakpoints next to chromosomal knob repeat arrays.
...
RNA-seq identified 465 differentially expressed genes, with the strongest signal from a JUMONJI histone demethylase cluster that shows a lineage-specific tandem expansion in B73 (five paralogs) relative to highland genomes (single ortholog).
...
Despite strong evidence linking \invfour to local adaptation, key aspects of its biology remain unresolved.
At the structural level, the inversion breakpoints have not been characterized at sequence resolution.
...
More broadly, whether \invfour effects are robust across field environments or reflect condition-specific responses remains untested.

Here, we characterize \invfour at multiple levels of organization.
We introgressed the highland haplotype from the Mexican traditional variety Mi21 into B73 through eight generations of backcrossing, generating Near Isogenic Lines (NILs).
These NILs were phenotyped across three field environments in Pennsylvania and North Carolina, with shoot apical meristem morphology measured in controlled conditions.
We used RNA-seq from field-grown leaves to identify differentially expressed genes and applied weighted gene coexpression network analysis (WGCNA) and trans-regulatory network reconstruction to characterize the regulatory architecture of the \invfour effect.
The phosphorus starvation response was evaluated as a companion study~\cite{rodriguez2026phosphorus}.

We show that \invfour breakpoints overlap chromosomal knob repeat arrays, consistent with an ectopic recombination origin.
\invfour accelerates flowering and increases plant height across environments, with genotype-by-environment interactions characteristic of local adaptation.
Transcriptomic analysis identifies a JUMONJI histone demethylase cluster with copy number variation between highland and lowland genomes as the strongest candidate.
WGCNA reveals widespread coexpression module disruption, with JMJ genes losing hub connectivity, and a trans-regulatory network connects chromatin regulators to cell proliferation genes.
These findings provide a mechanistic model linking \invfour to growth regulation through chromatin-mediated gene network rewiring.
...
Complete genome sequences of selected Mexican Highland traditional varieties allowed us to delimit the recombination breakpoints of \invfour by using an Anchorwave \cite{song2022} alignment of chromosome 4. With this aim, we aligned both PT, a close relative to the \invfour donor parent Mi21, as a representative genome for the Mexican highlands, and TIL18 representing the highland teosinte \mex, to B73, the recurrent parent used for \invfour introgression (Fig~\ref{fig::design}A, Supplementary Table~\ref{tab:breakpoints}).
...
Then we searched the two classes of knob repeats in TIL18 and PT, which have no public repetitive element annotation to date.
For comparison purposes, we did a BLAST+ \cite{camacho2009} similarity search of the three chromosomes 4 using a selected sequence of each repeat class as subjects (see methods).
