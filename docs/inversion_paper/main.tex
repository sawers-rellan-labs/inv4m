...
\textbf{(A)} Raw $\log_2$(CPM) expression of \textit{sec6} (\textit{Zm00001eb194380}) across leaf 3, 4, and 5 samples.
\textbf{(B)} Expression centered per genotype to highlight the divergence in leaf-position trajectory.
Lines connect genotype means $\pm$ SE; individual sample points are shown as jittered dots.
The \invfour karyotype shows a steeper decline from leaf 3 to leaf 5, consistent with the Leaf $\times$ Genotype interaction driving the greenyellow module connectivity loss described in Fig.~\ref{fig:wgcna}B and Supplementary Table~\ref{tab::greenyellow_module}.}
