...
Focusing instead on the direct effects of \invfour, we report four main findings.
First, we delimited \invfour by comparing the genome sequences of a Mi21-containing NIL and B73.
We found the inversion to span 15 Mb, with breakpoints bounded by chromosomal knob repeat regions (Fig~\ref{fig::design}C, Table~\ref{tab:breakpoints}).
...
Third, \invfour disrupts growth regulation gene networks as evidenced by WGCNA and trans-coexpression analysis (Table~\ref{tab::preservation}, Fig~\ref{fig:wgcna}C).
Fourth, the JMJ histone demethylase cluster emerges as the most statistically robust signal among differentially expressed genes (Fig~\ref{fig:jmj_cluster}).
...
In Atlantic cod, tandem arrays of hAT-derived elements at the breakpoints of megabase-scale inversions show a striking structural parallel to \invfour: the two breakpoint segments of each inversion differ in size and repeat orientation, with one segment carrying inverted tandem arrays and the other showing a simpler repeat structure \cite{araya2025}.
Whether a similar recombination process contributed to the origin of \invfour remains unknown.
