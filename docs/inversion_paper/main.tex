...
% TODO: Add the 3 component Z-statistics from Langfelder & Horvath 2011
% (Z_density, Z_connectivity, Z_separability) as additional columns.
% These are in the modulePreservation() output from 06_preservation.Rmd.
% TODO: Add a companion table for the topology control modules
% (photosynthesis/senescence, permutation_test_delta_kWithin.Rmd).
% Needs: LFP (run bootstrap on control modules), Z_summary components,
% delta_kWithin, FDR from permutation test.
% TODO: Run consensus module pipeline (step 04) on control gene set
% before bootstrap, so LFP comparison with DEG modules is fair.
% Current control modules use single cutreeDynamic (no consensus).
\begin{table}[!ht]
