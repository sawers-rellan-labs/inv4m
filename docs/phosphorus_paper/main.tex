...
BH-adjusted thresholds: \textit{n.s.}\ non significant, $\textit{FDR} < 0.05$ (*), $< 0.01$ (**), $< 0.001$ (***), $< 0.0001$ (****).
}
\label{fig:Figure_1}
\end{figure*}
\clearpage
...
Boxplots for phosphorus (P), zinc (Zn), calcium (Ca), and sulfur (S) show, per genotype, \textbf{(A)} element concentrations (mg/kg) in stover and grain, \textbf{(B)} grain/stover concentration ratios, \textbf{(C)} per plant content (mg/plant) in stover and grain, and \textbf{(D)} mineral harvest index (HI, grain/total).
BH-adjusted thresholds: $\textit{FDR} < 0.05$ (*), $< 0.01$ (**), $< 0.001$ (***), $< 0.0001$ (****); non significant brackets are not shown.
...
Sulfur metabolism was also affected: the chromosome 9 cysteine synthase tandem (\textit{cys3}/\textit{cys4}/\textit{cys8}, \textit{Zm00001eb376930}) was strongly repressed ($-\log_{10}{\text{FDR}} = 3.23$, $\log_2{\text{FC}} = -1.85$), driving the over-representation of the KEGG Sulfur cycle pathway (zma01320, adj.\,$P = 0.023$) among $-$P downregulated genes (Fig~\ref{fig:Figure_4}B).

\begin{figure*}[!ht]
\centering
\includegraphics[width=0.85\linewidth]{figs/Figure_4.png}
\caption{Controlled vocabulary enrichment. Overrepresentation analysis for (A) Gene ontology Biological Process, in parentheses, the total number of high confidence DEGs in each  category, the sum of annotated genes in all overrepresented terms per category is illustrated in \ref{supp:Figure_S6}~C-D,
...
Direct measurements of increased shoot zinc under phosphorus limitation have been documented in maize \citep{soltangheisi2014,saenchai2016}, sorghum \citep{oseni2009}, finger millet \citep{maharajan2023}, and \textit{Arabidopsis} under long-term Pi deprivation \citep{misson2005}.
...
The transcriptional data recover the sulfolipid arm of this cross-talk, with \textit{sqd1}, \textit{sqd2} and \textit{sqd3} among the strongest $-$P upregulated genes, consistent with SQDG biosynthesis upregulation seen across maize genotypes \citep{he2022}.
The chromosome 9 cysteine synthase tandem array (\textit{cys3}/\textit{cys4}/\textit{cys8} at \textit{Zm00001eb376930}, and the adjacent \textit{cys7}) is repressed under $-$P with the repression deepening in older leaves (Leaf$\times -$P interaction), and the Sulfur cycle pathway (KEGG zma01320) is overrepresented in both the $-$P and Leaf:$-$P downregulated clusters at this locus.
...
Samples were weighed, digested in double-distilled HNO$_3$ followed by a 60/40 nitric/perchloric acid mixture at 150\,\textdegree C, and diluted to a final volume of 10\,mL with deionised water prior to analysis on a Thermo iCap 7000 ICP-OES \cite{rahmatiishka2025}.
...
Significance brackets in Figure~\ref{fig:Figure_1} and in the ionome figures (Figures \ref{fig:Figure_2}, \ref{supp:Figure_S4}, \ref{supp:Figure_S5}) are pairwise contrasts from the same linear model (Equation~\ref{eq:pheno_model}), extracted with the \texttt{emmeans} package \cite{lenth2023}.
For the phenotype figure, we computed three $-$P vs $+$P contrasts per trait: the simple effect within each genotype, and the effect marginalised over genotypes.
All 48 contrasts (16 traits $\times$ 3) were pooled for a single Benjamini and Hochberg adjustment; per genotype contrasts are drawn in Figure~\ref{fig:Figure_1} and the marginal contrasts in Supplementary Figure~\ref{supp:Figure_S1}.
For the ionome figures, each of eight minerals was modelled separately across four metric families: concentration in grain and in stover (16 models), per plant content in grain and in stover (16 models), grain/stover concentration ratio (8 models), and mineral harvest index (8 models).
From every model we extracted the $-$P vs $+$P simple effect within each genotype, yielding 96 contrasts that were FDR-pooled across all four metric families and displayed as brackets in Figure~\ref{fig:Figure_2} (P, Zn, Ca, S) and Supplementary Figure~\ref{supp:Figure_S4} (Mg, Mn, K, Fe).
Supplementary Figure~\ref{supp:Figure_S5} instead shows the marginal \invfour vs CTRL contrast (48 contrasts, one per ionome model) FDR-pooled across the same four metric families.
