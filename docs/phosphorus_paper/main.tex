...
Aggregate lipid class composition reveals the expected dominance of PC, MGDG, DGDG, and SQDG in membrane lipids, with phosphorus deficiency causing notable decreases in phosphoglycerolipids across all leaf stages and contrasting developmental trajectories for neutral lipids under $+$P versus $-$P conditions (\ref{supp:Figure_S7}).
...
The three genes showing significant genotype-by-phosphorus interactions (\textit{aldh2}, \textit{gras80}, and \textit{flz22}, \ref{supp:Figure_S6}) are located outside the inversion boundaries and did not show additional interactions with leaf stage.
...
Lipid models additionally included MS injection order as a technical covariate (\ref{supp:Figure_S8}).
...
Brackets correspond to the \invfour $-$ CTRL contrast from \texttt{emmeans} on the per response linear model $y \sim \text{Genotype} \times \text{Treatment}$, pooled across phosphorus treatments, with Benjamini and Hochberg \textit{FDR} applied once across all 20 cells: $\textit{FDR} < 0.05$ (*), $< 0.01$ (**), $< 0.001$ (***), $< 0.0001$ (****); non significant brackets are not drawn.}
