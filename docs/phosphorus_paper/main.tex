...
Phosphorus deficiency delayed flowering, reduced biomass, and decreased yield (Fig~\ref{fig:Figure_1}) in sibling NILs homozygous for the B73 reference (CTRL) and \invfour-MICH21 karyotypes alike.
Under $-P$ conditions, anthesis and silking occurred more than three days later relative to $+P$ (Fig~\ref{fig:Figure_1}A and B; anthesis: $3.60 \pm 0.26$~days, FDR $= 1.1 \times 10^{-18}$; silking: $3.42 \pm 0.23$~days, FDR $= 8.1 \times 10^{-20}$; marginal effect estimate $\pm$ S.E, FDR adjusted across the 144-test phenotype family).
In reduced phosphorus, the 50-kernel weight decreased by nearly $18\%$ ($-1.86 \pm 0.37$~g, FDR $= 4.1 \times 10^{-5}$, Fig~\ref{fig:Figure_1}C).
Under $-$P, 50-kernel weight decreased in both CTRL ($-1.80$~g, FDR $= 0.005$) and \invfour ($-1.92$~g, FDR $= 0.002$) plants, total kernel weight per plant remained statistically unchanged in either genotype, and total kernel number increased only in the marginal contrast pooled over genotype ($+58.7$, FDR $= 0.044$; \ref{supp:Figure_S1}).
Biomass accumulation was diminished under phosphorus starvation at all measured time points (Fig~\ref{fig:Figure_1}G): stover dry weight declined by $5.09 \pm 0.71$~g at 40~DAP (FDR $= 1.1 \times 10^{-8}$), $16.45 \pm 1.11$~g at 50~DAP (FDR $= 1.1 \times 10^{-18}$), and $27.94 \pm 3.47$~g at 60~DAP (FDR $= 1.4 \times 10^{-9}$).
By harvest time (121~DAP), stover biomass remained around $18.5\%$ lower ($-18.70 \pm 3.27$~g, FDR $= 2.3 \times 10^{-6}$).
...
Specifically, while the cob diameter of control lines was decreased less in P-starved plants ($-0.19 \pm 0.70$ cm, FDR $= 0.85$), the $\invfour$ plants grew a cob $10.7\%$ thinner under nutrient limitation ($-2.81 \pm 0.68$ cm, FDR $= 6.2 \times 10^{-4}$; per-genotype effect estimate $\pm$ S.E, FDR adjusted across the 144-test phenotype family, Fig~\ref{fig:Figure_1}F).
...
Pooled across treatments, \invfour plants flowered earlier (anthesis: $-1.31 \pm 0.26$~days, FDR $= 2.5 \times 10^{-5}$; silking: $-0.93 \pm 0.23$~days, FDR $= 6.0 \times 10^{-4}$) and grew taller by $6.41 \pm 1.05$~cm (FDR $= 5.0 \times 10^{-7}$); marginal \invfour vs CTRL effects from the 144-test phenotype family, \ref{supp:Figure_S2}.
...
Furthermore, downregulated genes consistent with reduced photosynthetic activity included light harvesting components such as \textit{lhcb10}, and the stress responsive RUBISCO activase \textit{rca3} ($-\log_{10}{FDR}=3.8$, $\log_2{FC}=-3.40$).
...
Concurrent with this phospholipid to galactolipid swap, transcriptome data show induction of \textit{pah1} (phosphatidate phosphatase, $\log_2{\text{FC}}=+1.30$) and \textit{dgk6} (diacylglycerol kinase, $\log_2{\text{FC}}=+0.46$), which produce the diacylglycerol substrate for galactolipid synthesis, alongside repression of phospholipase D paralogs (\textit{pld10}, \textit{pld13}, \textit{pld15}) and a chloroplastic Phospholipase A1.
Downregulated genes included \textit{peamt2} (\textit{Zm00001eb294690}), which catalyzes phosphoethanolamine methylation in the Kennedy pathway for PC biosynthesis, the light harvesting gene \textit{lhcb10} (\textit{Zm00001eb357740}) and the stress responsive RUBISCO activase \textit{rca3} (\textit{Zm00001eb164380}), and transcription factors \textit{zim25} (\textit{Zm00001eb278320}), \textit{nactf132} (\textit{Zm00001eb324550}), and \textit{bzip81} (\textit{Zm00001eb198410}).
...
To map the regulatory architecture governing these processes, we compiled a list of SAGs by cross-referencing our strong DEGs with manually curated sources \cite{zhang2014c,liu2011,ojedarivera2026,berardini2015,durinck2005}, resulting in a total of 110 genes (\nameref{S1_File}). 
...
Typical senescence regulators like \textit{nactf108} and \textit{nactf44} (\textit{Zm00001eb015630}) consistently increase with leaf age, and \textit{nactf132} (\textit{Zm00001eb324550}, \textit{ZmNAC132}) shares this leaf-age upregulation while also showing strong downregulation under phosphorus stress.
...
The cys3/cys4 product physically interacts with the E3 ubiquitin ligase NLA in yeast two-hybrid assays \citep{liao2026}, a regulator of phosphate homeostasis that drives ubiquitination and turnover of PHT1 transporters together with the E2 conjugating enzyme PHO2 \citep{huang2013}, raising the possibility of a post-translational link between cysteine biosynthesis and Pi transporter regulation beyond the transcriptional repression observed here.
...
Across whole plant phenotypes, the leaf transcriptome, lipidome, and ionome, phosphorus deficiency resulted in consistent starvation responses.
However, we did not find decisive evidence that the \invfour inversion modulates any of them.
The two exceptions were incidental: a significant \invfour $\times$ phosphorus interaction on cob diameter (Fig.~\ref{fig:Figure_1}F) and on the expression of \textit{aldh2} (\ref{supp:Figure_S7}), a gene located in the flanking introgression of the inversion and without annotation related to phosphorus.
No candidate gene for the phosphorus starvation response residing within the inversion showed a significant \invfour $\times -$P interaction at the transcriptional level.
This includes \textit{phos2} (\textit{Zm00001eb191650}, ZmPHO1;2a), a PHO1 family phosphate efflux transporter and a candidate for adaptation to low phosphorus availability within the inversion \cite{salazar-vidal2016, ma2021,alonsonieves2022, zhang2025}.
...
Nonetheless, we detected additive \invfour effects on the ionome, most prominently reductions in stover Ca, Zn, and K content and differences in Fe grain content and harvest index (\ref{supp:Figure_S5}).
However, \invfour plants showed lower stover biomass at harvest (marginal Inv4m vs CTRL contrast: $-7.9 \pm 3.3$~g STWHV, FDR $= 0.044$), so the stover content differences may in part reflect this biomass difference rather than a distinct mineral reallocation mechanism.
...
For each of the 16 field plant phenotype traits we extracted from Eq.~\ref{eq:pheno_model} (\texttt{emmeans} \cite{lenth2023}) the nine contrasts that span the $2 \times 2$ \invfour $\times$ phosphorus design: the $-$P vs $+$P contrast at each genotype, the \invfour vs CTRL contrast at each treatment, the two diagonal contrasts, the marginal Treatment and Genotype contrasts, and the Treatment $\times$ Genotype interaction. These 144 contrasts were combined into a single family prior to Benjamini-Hochberg FDR adjustment.
...
To visualize developmental and stress-induced changes in major biological processes, we calculated transcriptomic indices for four gene sets.
The Chlorophyll Synthesis set was compiled from CornCyc pathway annotations.
The Chlorophyll Degradation set was manually curated from the same CornCyc reactions, supplemented with six additional genes drawn from KEGG and \cite{wei2025} that mediate canonical chlorophyll catabolism in maize but were missing from the CornCyc reaction set.
The Photosynthesis (163 genes) and Leaf Senescence (55 genes) sets were taken from \cite{ojedarivera2026}, who curated panicoid grass orthologs of canonical photosynthesis and senescence regulators.
