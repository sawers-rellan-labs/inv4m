...
# one row per plot with {mineral}_seed and {mineral}_stover columns. Apply
...
ionome_wide <- read.csv(ionome_csv, na.strings = c("", "NA")) %>%
  mutate(tissue = recode(tissue, "stalk" = "stover")) %>%
...
for (ion in reliable_minerals) {
  seed_col   <- paste0(ion, "_seed")
  stover_col <- paste0(ion, "_stover")
  field_data_complete[[paste0(ion, "_grain_mg")]] <-
    field_data_complete$TKW * field_data_complete[[seed_col]] / 1000
...
ear_phenotypes <- c("EW", "EL", "ED", "KRN", "CD", "TKW", "KW50", "TKN",
                    "HI", "biomass_HI_frac")

ionome_conc_phenotypes <- c(
  paste0(reliable_minerals, "_seed"),
