...
3. **Ratio Analysis**: Grain:Stover ratios for each mineral
4. **Interaction Analysis**: Genotype × Treatment interactions

Each mineral plot uses nested facets showing: Tissue (stover/grain) > Genotype (CTRL/Inv4m), with Treatment on the x-axis.
...
```{r load-data}
# Read spatially corrected mineral concentrations (includes ionome data)
ionome <- read.csv(file.path(paths$intermediate, "PSU2022_spatially_corrected_both_treatments_with_ionome.csv"))

# Rename for consistency and filter for Inv4m and CTRL genotypes only.
# Fe_stover > 800 and Mn_stover > 75 outliers are masked upstream in
# spatial_correction_for_INV4MXP.Rmd before SpATS fitting, so no further
...
to_plot <- ionome %>%
  ...
  filter(tissue %in% c("grain", "stover")) %>%
  mutate(
    tissue = factor(tissue, levels = c("stover", "grain")),
...
When exporting t-test results to CSV, we remove list columns that arise from using `group_modify()` with `rstatix::add_y_position()`. These list columns store grouping metadata needed for plotting significance brackets but cannot be serialized to CSV format. The full data with `y.position` values remains in memory for plotting.

```{r save-plots}
# Save main combined figure (content only)
ggsave(
  file.path(paths$figures, "ionome_main_figure.pdf"),
...
ggsave(
  file.path(paths$figures, "ionome_main_figure.png"),
  plot = p_main_combined,
  width = 12,
  height = 4,
  units = "in",
...
if (exists("p_main_with_ratios")) {
  ggsave(
    file.path(paths$figures, "ionome_main_with_ratios.pdf"),
  ...
  ggsave(
    file.path(paths$figures, "ionome_main_with_ratios.png"),
    plot = p_main_with_ratios,
    width = 12,
    height = 8,
    units = "in",
...
ggsave(
  file.path(paths$figures, "ionome_supplementary_figure.pdf"),
...
ggsave(
  file.path(paths$figures, "ionome_supplementary_figure.png"),
  plot = p_supp_combined,
  width = 12,
  height = 4,
  units = "in",
...
if (exists("p_supp_with_ratios")) {
  ggsave(
    file.path(paths$figures, "ionome_supplementary_with_ratios.pdf"),
  ...
  ggsave(
    file.path(paths$figures, "ionome_supplementary_with_ratios.png"),
    plot = p_supp_with_ratios,
    width = 12,
    height = 8,
    units = "in",
...
# Export CSV files
write.csv(effects, file.path(paths$intermediate, "ionome_linear_model_effects.csv"), row.names = FALSE)
write.csv(significant, file.path(paths$intermediate, "ionome_significant_effects.csv"), row.names = FALSE)
write.csv(interaction_effects, file.path(paths$intermediate, "ionome_interaction_effects.csv"), row.names = FALSE)

# Drop y position list columns - CSV cannot serialize them
treatment_export <- treatment.t.test %>%
  select(where(~!is.list(.)))
write.csv(treatment_export, file.path(paths$intermediate, "ionome_treatment_ttests.csv"), row.names = FALSE)

if (nrow(ratio.t.test) > 0) {
  ratio_export <- ratio.t.test %>%
    select(where(~!is.list(.)))
  write.csv(ratio_export, file.path(paths$intermediate, "ionome_ratio_ttests.csv"), row.names = FALSE)
}
```

## Pairwise Genotype Comparisons for Significant Effects
```{r genotype-pairwise-comparisons, fig.width=8, fig.height=4}
...
mg_grain_data <- to_plot %>%
  filter(mineral == "Mg", tissue == "grain") %>%
  mutate(
    Genotype = factor(Genotype, levels = c("CTRL", "<i>Inv4m</i>")),
    Treatment = factor(Treatment, levels = c("+P", "-P"))
  )
...
genotype.t.test <- bind_rows(
  mg_grain_data %>% mutate(mineral_tissue = "Mg_grain"),
...
if (nrow(genotype.t.test) > 0) {
  genotype.t.test <- genotype.t.test %>%
    group_by(mineral_tissue, Treatment) %>%
    group_modify(~ {
      if (.y$mineral_tissue == "Mg_grain") {
        test_data <- mg_grain_data %>%
...
mg_grain_genotype_test <- genotype.t.test %>%
  filter(mineral_tissue == "Mg_grain")

ca_stover_genotype_test <- genotype.t.test %>%
  filter(mineral_tissue == "Ca_stover")

# Plot for Mg grain
p_mg_grain_genotype <- ggplot(
  mg_grain_data, 
...
cat("\n=== Genotype Effects on Mg (Grain) ===\n")
print(mg_grain_genotype_test)

cat("\n=== Genotype Effects on Ca (Stover) ===\n")
print(ca_stover_genotype_test)
```
```{r save-genotype-comparison}
# Save genotype comparison figure
ggsave(
  file.path(paths$figures, "ionome_genotype_effects.pdf"),
...
ggsave(
  file.path(paths$figures, "ionome_genotype_effects.png"),
  plot = p_genotype_effects,
  width = 8,
  height = 4,
  units = "in",
...
# Drop y position list columns - CSV cannot serialize them
genotype_export <- genotype.t.test %>%
  select(where(~!is.list(.)))
write.csv(
  genotype_export,
  file.path(paths$intermediate, "ionome_genotype_comparison_ttests.csv"),
  row.names = FALSE
)
```
## Session Info
