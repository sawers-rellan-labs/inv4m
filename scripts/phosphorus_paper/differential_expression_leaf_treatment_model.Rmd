...
This model describes the log-transformed expression $y_{g,i}$ of gene $g$ in sample $i$ as a function of spatial, developmental, and treatment effects.
Each gene is fitted separately using a common design matrix, producing gene-specific regression coefficients.

$$
\begin{aligned}
y_{g,i} &= \beta_{g,0} + \beta_{g,R}\text{Row}_i + \beta_{g,L}\text{leafc}_i + \beta_{g,T}T_i \\
&\quad + \beta_{g,L\times T}(\text{leafc}_i \times T_i) + \beta_{g,G}G_i \\
&\quad + \beta_{g,G\times T}(G_i \times T_i) + \beta_{g,G\times L}(G_i \times \text{leafc}_i) \\
&\quad + u_{g,\text{col}(i):T(i)} + \varepsilon_{g,i}
\end{aligned}
$$
