...
For each trait, all 9 contrasts spanning the 2x2 \invfour x phosphorus
design are estimated from `lm(y ~ Genotype * Treatment)` via emmeans
custom contrasts:
...
All 144 contrasts (16 traits x 9) are FDR-adjusted in a single pool and
saved to `PSU2022_fig1_bracket_contrasts.csv`. Backward-compatible columns
`contrast_level` ("per_Genotype" or "marginal" for the original 3 contrast
types; new values for the additional 6) and `Genotype` (CTRL/Inv4m for
per_Genotype rows; NA otherwise) preserve filters used by the figure
scripts.
