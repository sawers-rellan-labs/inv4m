...
### Stage 3: Pairwise ±P contrasts for figure brackets

For each trait, per-Genotype and marginal ±P contrasts are estimated from
`lm(y ~ Genotype * Treatment)` via emmeans. All 48 contrasts (16 traits ×
3: CTRL, Inv4m, marginal) are FDR-adjusted in a single pool and saved to
`PSU2022_fig1_bracket_contrasts.csv` for use by the plotting scripts.

```{r pairwise-contrasts}
contrasts <- bind_rows(
  lapply(vars, function(v) {
    if (sum(!is.na(pheno[[v]])) < 20) return(NULL)
    m <- tryCatch(lm(as.formula(paste(v, "~ Genotype * Treatment")), data = pheno),
                  error = function(e) NULL)
    if (is.null(m)) return(NULL)
    bind_rows(
      pairs(emmeans(m, ~ Treatment | Genotype), reverse = TRUE) %>%
        as_tibble() %>% mutate(trait = v, contrast_level = "per_Genotype"),
      pairs(emmeans(m, ~ Treatment),            reverse = TRUE) %>%
        as_tibble() %>% mutate(trait = v, contrast_level = "marginal")
    )
  })
) %>%
...
cat("±P contrasts — per-Genotype and marginal, FDR across 48:\n")
contrasts %>%
...
cat("±P contrasts with % change over CTRL +P mean:\n")
bracket_contrasts %>%
...
cat("Saved", nrow(bracket_contrasts), "contrasts to PSU2022_fig1_bracket_contrasts.csv\n")
```
...
```{r supp_biomass, fig.width=12, fig.height=12}
...
```{r supp_inv4m_by_treatment_color_fixed, fig.width=14, fig.height=7}
...
Boxplots pool CTRL and Inv4m; bracket labels come from the marginal emmeans contrast
FDR (48-contrast D pool). PH has a robust marginal Treatment effect and non-significant
Genotype × Treatment interaction. KW50, TKN, and TKW are shown together to address
Sawers' Figure 1 comment: under $-P$ the plant trades kernel size for kernel number
(KW50 drops significantly, TKN rises significantly in the pooled marginal contrast),
leaving total kernel weight (TKW) statistically unchanged.
