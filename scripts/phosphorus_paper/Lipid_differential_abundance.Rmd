...
# New metadata already has Rep, Plot_Row, Plot_Column - only get DTA/DTS from psu
sampleInfo <- psu %>%
  select(rowid, DTA, DTS) %>%
  inner_join(sampleInfo, by = "rowid") %>%
  filter(!is.na(DTA))
...
sampleInfo$tube <- gsub("L|R", "S", sampleInfo$top_tag, perl = TRUE)

ms_order <- read.csv(file.path(paths$data, "PSU-PHO22_ms_order.csv"))
ms_order$tube <- gsub("L|R", "S", ms_order$top_tag, perl = TRUE)

# Only get injection_order from ms_order to avoid duplicate columns
sampleInfo <- sampleInfo %>%
  dplyr::inner_join(ms_order %>% select(tube, injection_order), by = "tube") %>%
  distinct()
...
# Save combined plot
ggsave(
  file.path(paths$figures, "sup_fig_7.png"),
  sup_fig,
  width = 16,
  height = 9,
  dpi = 150
...
- Tier 2: Strong (significant + effect size threshold)
...
cat("\nTier 2 - Strong DLs:\n")
print(table(effects_df$predictor, effects_df$is_strong_DL))
...
write.csv(
  effects_df,
  file = file.path(paths$intermediate, "lipid_differential_abundance_TIC_normalized.csv"),
  row.names = FALSE
)

write.csv(
  molar_ion_fractions,
  file = file.path(paths$intermediate, "lipid_molar_ion_fractions.csv"),
  row.names = TRUE
)

write.csv(
  voomR$E,
  file = file.path(paths$intermediate, "voom_log2_TIC_normalized_lipids.csv"),
  row.names = TRUE
)

cat("=== Analysis Complete ===\n")
cat("Results exported to intermediate/\n")
```
...
ggsave(
  filename = file.path(paths$figures, "lipid_volcano_3_TIC.png"),
...
saveRDS(lipid_volcano_3, file.path(paths$intermediate, "lipid_volcano_3_TIC.rds"))
...
print(volcano_2)
saveRDS(volcano_2, file.path(paths$intermediate, "lipid_volcano_2_TIC.rds"))
ggsave(
  filename = file.path(paths$figures, "lipid_volcano_2_TIC.png"),
  plot = volcano_2,
  height = 6,
  width = 10,
  units = "in",
...
ggsave(
  filename = file.path(paths$figures, "lipid_volcano_leafP_TIC.png"),
  plot = volcano_leafP,
  height = 7,
  width = 7,
  units = "in",
...
## Export Strong Lipids
```{r export_strong_lipids}
write.csv(
  label_data,
  file = file.path(paths$intermediate, "strong_DLs_TIC.csv"),
  row.names = FALSE
)

cat("Volcano plots saved to figures/\n")
cat("Strong lipids saved to intermediate/\n")
```
...
**Purpose**: Extract strong lipids showing significant interaction
between leaf developmental stage and phosphorus treatment.
```{r identify_interaction_lipids}
interaction_lipids <- effects_df %>%
  filter(predictor == "Leaf:-P", is_strong_DL) %>%
  arrange(adj.P.Val)

cat("Strong Leaf:-P interaction lipids:",
...
**Purpose**: Visualize strong lipids showing significant interaction
...
ggsave(
  filename = file.path(paths$figures, "lipid_trajectory_6panel.png"),
  plot = p_trajectory_6,
  width = 18,
  height = 6,
  dpi = 300
...
saveRDS(p_trajectory_6, file.path(paths$intermediate, "lipid_trajectory_6panel.rds"))
