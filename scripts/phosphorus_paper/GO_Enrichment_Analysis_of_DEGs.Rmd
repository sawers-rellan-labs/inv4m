...
TERM2GENE <- readRDS(file.path(paths$data, "TERM2GENE_Fattel_2024_full.rds"))
...
p_coverage <- ggplot(coverage_plot_data, aes(x = Cluster_label, y = Count, fill = fill_color)) +
  ...
  labs(x = NULL, y = "Number of genes", title = "Strong DEGs\nin enriched GO BP terms") +
  theme_classic(base_size = 25) +
  theme(
    plot.title = element_text(size = 28, face = "bold", hjust = 0.5),
    axis.text.y = element_text(size = 25),
...
p_venn_annotated_gg <- as.ggplot(p_venn_annotated) +
  labs(title = sprintf("Strong DEGs\nin enriched GO BP terms", 
...
minusP_0016036 <- effects_df %>%
  filter(is_strong_DEG,gene %in% p_annotation$geneID & predictor=="-P") %>%
...
leafxP_0016036 <- effects_df %>%
  filter(is_strong_DEG,gene %in% p_annotation$geneID & predictor=="Leaf:-P") %>%
  dplyr::select(predictor, regulation,gene,locus_label, logFC, neglogP, desc_merged)
