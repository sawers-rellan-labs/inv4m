---
title: "Genotype × Environment Analysis of inv4m Introgression Effects: NC2025 vs PA2025"
...
- **PA2025**: Pennsylvania environment (northern)
- **NC2025**: North Carolina environment (southern)
...
cat("PA2025 dimensions:", dim(psu2025), "\n")
cat("NC2025 dimensions:", dim(cly2025), "\n")
cat("PA2025 genotypes:", paste(unique(psu2025$genotype), collapse = ", "), "\n")
cat("NC2025 genotypes:", paste(unique(cly2025$genotype), collapse = ", "), "\n")
cat("PA2025 donors:", paste(unique(psu2025$donor), collapse = ", "), "\n")
cat("NC2025 donors:", paste(unique(cly2025$donor), collapse = ", "), "\n")
...
cat("PA2025 original donors:", paste(psu2025_donors_orig, collapse = ", "), "\n")
cat("NC2025 original donors:", paste(cly2025_donors_orig, collapse = ", "), "\n")
...
### 2.1 PA2025 Analysis by Donor

```{r psu2025-analysis}
psu2025_results_list <- list()

for (current_donor in c("TMEX", "MI21")) {
  cat("\n=== PA2025", current_donor, "Analysis ===\n")
  ...
  psu_summary <- psu_results %>%
    ...
    mutate(donor = current_donor, environment = "PA2025")
...
### 2.2 NC2025 Analysis by Donor

```{r cly2025-analysis}
cly2025_results_list <- list()

for (current_donor in c("TMEX", "MI21")) {
  cat("\n=== NC2025", current_donor, "Analysis ===\n")
  ...
  cly_summary <- cly_results %>%
    ...
    mutate(donor = current_donor, environment = "NC2025")
...
if (length(common_donors) == 0) {
  stop("No common donors found between PA2025 and NC2025")
}
```
...
for (donor in common_donors) {
  ...
  psu_donor <- psu2025 %>%
    ...
    select(plotId, genotype, all_of(shared_phenotypes)) %>%
    mutate(environment = "PA2025", donor = donor) %>%
    filter(complete.cases(.))

  cly_donor <- cly2025 %>%
    filter(donor == !!donor, genotype %in% c("CTRL", "INV4M")) %>%
    select(plotId, genotype, all_of(shared_phenotypes)) %>%
    mutate(environment = "NC2025", donor = donor) %>%
    filter(complete.cases(.))

  donor_gxe <- bind_rows(psu_donor, cly_donor) %>%
    mutate(
      genotype = factor(genotype, levels = c("CTRL", "INV4M")),
      environment = factor(environment, levels = c("NC2025", "PA2025"))
    )
...
forest_plot <- ggplot(all_effect_sizes, 
  ...
  geom_text(aes(label = paste0("(", magnitude, ")")),
            ...
            hjust = -0.2, size = 3) +
  scale_color_manual(values = c("NC2025" = "tomato", "PA2025" = "royalblue")) +
  labs(
...
cat("1. PA2025 Results by Donor:\n")
if (nrow(psu2025_all_results) > 0) {
  print(psu2025_all_results)
} else {
  cat("No PA2025 results available\n")
}

cat("\n2. NC2025 Results by Donor:\n")
if (nrow(cly2025_all_results) > 0) {
  print(cly2025_all_results)
} else {
  cat("No NC2025 results available\n")
}
