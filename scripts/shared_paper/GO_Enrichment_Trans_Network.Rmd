...
**Approach**:
1. Load trans network gene sets from MaizeNetome analysis (strong DEGs: FDR < 0.05, |log2FC| > 1.5)
...
trans_regulators <- read.csv("~/Desktop/trans_network_regulators.csv")
trans_targets <- read.csv("~/Desktop/trans_network_targets.csv")
trans_network_all <- read.csv("~/Desktop/trans_network_all_genes.csv")
trans_edges <- read.csv("~/Desktop/trans_network_edges_annotated.csv")
...
cat("Strong DEGs:", sum(effects_df$is_strong_DEG), "\n")
```
...
cat("DEG categories:\n")
cat("  Strong DEGs (is_strong_DEG):", sum(effects_df$is_strong_DEG), "\n")
...
ggsave(
  annotation_plot,
  file = "~/Desktop/GO_trans_network_annotated.svg",
  height = 10,
  width = 14
)
...
ggsave(
  annotation_plot,
  file = "~/Desktop/GO_trans_network_annotated.pdf",
  height = 10,
  width = 14
)
...
# Compare Strong vs Top DEGs

**Purpose**: Compare enrichment patterns between strong network DEGs
(|log2FC| > 1.5) and Top DEGs (|log2FC| > 2).

```{r compare_strong_vs_top}
# Common terms
strong_terms <- slim_comp@compareClusterResult$ID
top_terms <- slim_top@compareClusterResult$ID

common_terms <- intersect(strong_terms, top_terms)
strong_only <- setdiff(strong_terms, top_terms)
top_only <- setdiff(top_terms, strong_terms)

cat("Enrichment comparison:\n")
cat("  Terms in strong DEGs:", length(strong_terms), "\n")
cat("  Terms in Top DEGs:", length(top_terms), "\n")
cat("  Common terms:", length(common_terms), "\n")
cat("  Strong only:", length(strong_only), "\n")
...
write.csv(
  ...
  file = "~/Desktop/GO_enrichment_trans_network_all.csv",
  row.names = FALSE
)

write.csv(
  slim_top@compareClusterResult,
  file = "~/Desktop/GO_enrichment_trans_network_topDEGs.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file = "~/Desktop/GO_gene_term_associations.csv",
  row.names = FALSE
)
...
ggsave(
  annotation_plot,
  file = "~/Desktop/Network_GO_annotation_plot.svg",
  height = 8,
  width = 12
)

cat("\nFiles exported:\n")
cat("  ~/Desktop/GO_enrichment_trans_network_all.csv\n")
cat("  ~/Desktop/GO_enrichment_trans_network_topDEGs.csv\n")
cat("  ~/Desktop/GO_gene_term_associations.csv\n")
cat("  ~/Desktop/Network_GO_annotation_plot.svg\n")
```
