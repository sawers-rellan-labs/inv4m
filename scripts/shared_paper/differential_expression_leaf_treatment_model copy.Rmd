...
(Plot_Row, Plot_Column), biological factors (Genotype, leaf_tissue, Treatment), 
and the leaf_tissue:Treatment interaction. Voom transformation estimates 
mean-variance relationships and computes precision weights for each observation.
...
design <- model.matrix(
  ~ Plot_Column + Plot_Row + Genotype + leaf_tissue * Treatment,
  d
)
...
The goal of this section is to classify differentially expressed genes (DEGs) into 
three tiers of stringency based on significance (FDR) and effect size ($\log_2\text{FC}$), 
as well as a final set of genes selected for deep-dive analysis.

## Methodology for Fold Change Thresholds

The effect sizes are interpreted differently based on the predictor:

**Leaf Position (leaf predictor)**: This was modeled as a continuous slope ($\beta$). 
To be considered a large effect, the total change across the 3 units 
(Leaf 1 $\rightarrow$ Leaf 4) must meet the $|\log_2\text{FC}| > 2$ criterion.

$$\\ |\text{Total Change}| = |3\beta| > 2 \implies |\beta| > \frac{2}{3} \approx 0.67$$

We use $|\beta| > 0.7$ as the threshold for the leaf slope.

**Categorical Predictors (-P, Inv4m, Interaction)**: The standard large effect 
size threshold of $|\log_2\text{FC}| > 2.0$ is applied directly.

## Three-Tier Definitions

- **Significant DEGs** (is_DEG): All genes with FDR (adjusted P-value) $< 0.05$.

- **High-confidence DEGs** (is_hiconf_DEG): Significant DEGs that also meet the 
  large effect size thresholds defined above.

- **Selected DEGs** (is_selected_DEG): The *union* of the top 20 genes ranked by 
  P-value and the top 20 high-confidence genes ranked by the Mahalanobis distance 
  to the non-significant centroid near the origin of the 
  $(log_2(FC) \times-log_{10}FDR)$ plane.

## Significant DEGs

This first tier identifies all genes that pass the statistical significance 
threshold, regardless of effect size. We define is_DEG as any gene where the 
adjusted P-value (is_significant) is less than 0.05.
...
The second tier filters the significant DEGs further by applying the custom large 
effect size thresholds. This step defines is_hiconf_DEG as only those genes that 
are significant AND meet the $\log_2\text{FC}$ threshold specific to their 
predictor type (0.7 for leaf slope, 2.0 for categorical terms).
...
The final tier, is_selected_DEG, selects the most interesting genes for 
visualization and detailed annotation. A gene is selected if it is among the 
top N by P-value (among all DEGs) OR among the top N by Mahalanobis distance 
(among high-confidence DEGs). This step requires calculating intra-group rankings 
first.
...
write.csv(
  effects_df,
  file = "~/Desktop/predictor_effects_leaf_trt.csv",
  row.names = FALSE
)

cat("\nFull effects table exported:\n")
cat("  predictor_effects_leaf_trt.csv\n")
```
...
write.csv(
  selected_degs,
  file = "~/Desktop/selected_DEGs_leaf_trt.csv",
  row.names = FALSE
)
...
write.csv(
  p_selected,
  file = "~/Desktop/phosphorus_selected_DEGs_leaf_trt.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file = "~/Desktop/inv4m_selected_DEGs_leaf_trt.csv",
  row.names = FALSE
)
...
write.csv(
  interaction_selected,
  file = "~/Desktop/leaf_P_interaction_selected_DEGs.csv",
  row.names = FALSE
)

cat("\nSelected DEG tables exported:\n")
cat("  selected_DEGs_leaf_trt.csv - All selected DEGs\n")
cat("  phosphorus_selected_DEGs_leaf_trt.csv - Phosphorus effect\n")
cat("  inv4m_selected_DEGs_leaf_trt.csv - Inv4m effect\n")
cat("  leaf_P_interaction_selected_DEGs.csv - Leaf:Treatment interaction\n")
```
...
1. **Inv4m genotype**: Genes with different expression in Inv4m vs Control 
   (|logFC| > 2, FDR < 0.05)

2. **Leaf position gradient**: Genes showing expression changes along the 
   apical-basal axis (|logFC| > 0.7, FDR < 0.05)

3. **Phosphorus deficiency**: Genes responding to low P treatment 
   (|logFC| > 2, FDR < 0.05)

4. **Leaf × Treatment interaction**: Genes where the leaf gradient differs 
   between P treatments (|logFC| > 2, FDR < 0.05)
...
- **Region enrichment**: DEGs show enrichment in the Inv4m region, with patterns 
  observed across both inversion and flanking regions

- **Three-tier classification**:
  - Significant DEGs (FDR < 0.05): `r sum(effects_df$is_DEG)` genes
  - High-confidence DEGs (FDR < 0.05, |logFC| > 2): 
    `r sum(effects_df$is_hiconf_DEG)` genes
  - Selected DEGs (top `r rank_threshold` by significance/Mahalanobis): 
    `r sum(effects_df$is_selected_DEG)` genes
