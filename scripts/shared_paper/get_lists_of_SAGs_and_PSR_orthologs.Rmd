...
```{r join_expression}
# Prepare effects data for joining - filter for strong DEGs
effects_leaf <- effects_df %>%
  filter(is_strong_DEG == TRUE, predictor %in% c("Leaf", "Leaf:-P")) %>%
  dplyr::select(
    gene,
    predictor, 
    regulation, 
...
effects_psr <- effects_df %>%
  filter(is_strong_DEG == TRUE, predictor %in% c("-P", "Leaf:-P")) %>%
  dplyr::select(
    gene,
    predictor, 
    regulation, 
