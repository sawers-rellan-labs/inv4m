...
multiple sources, comparing them with strong DEGs from current study.
...
sag_orthologs_Zhang2014 <- read.csv(
  "~/Desktop/SAG_orthologs_Zhang2014.csv"
) %>%
  janitor::clean_names() %>%
...
effects_df %>%
  ...
  select(predictor, regulation, gene:logFC, std_err, adj.P.Val, 
         desc_merged) %>%
  group_by(predictor) %>%
...
sag_gwas_Sekhon2019 <- read.csv(
  "~/Desktop/senescence_Sekhon2019.csv"
) %>%
...
natural_senescence <- read.csv(
  "~/Desktop/natural_senescence.csv"
) %>%
  janitor::clean_names() %>%
...
sag_hub_ojeda2026 <- read.table(
  "~/Desktop/senesence_hubs_ojeda2026.tab",
  header = TRUE,
  sep = "\t",
  fill = TRUE
)
...
sag_hubs <- sag_hub_ojeda2026 %>% 
  ...
  select(predictor, regulation,v3,v4,gene,locus_label,symbol,functional_process, logFC, std_err, adj.P.Val, 
         desc_merged) %>%
  ungroup() %>%
...
write.csv(sag_hubs,
          "~/Desktop/sag_hub_ojeda2026_strong_degs.csv",
          row.names = FALSE)
nrow(sag_hubs)
```
...
photosynthesis_ojeda2026 <- read.table(
  "~/Desktop/photosynthesis_expression_index_genes_ojeda2026.txt",
  header = FALSE,
  sep = "\t"
)
...
photo_ojeda2026 <- effects_df %>%
  ...
  select(predictor, regulation, gene,locus_label, logFC, std_err, adj.P.Val, 
         desc_merged) %>%
  ungroup() %>%
  group_by(predictor, regulation) %>%
  arrange(predictor, regulation, adj.P.Val) 
...
write.csv(photo_ojeda2026,
          "~/Desktop/photo_ojeda2026_strong_degs.csv",
          row.names = FALSE)
...
effects_df %>%
  ...
  select(gene:logFC,std_err,adj.P.Val,desc_merged) %>% tibble()
...
```{r left_panel_c, fig.width=9, fig.height=7.5}
...
#' @param filter_strong Filter to strong DEGs
...
get_annotation_degs <- function(annotation_id,
  ...
  if (filter_strong) {
    result <- result %>% filter(is_strong_DEG)
  }
...
```{r run_enrichment}
DEGs <- with(effects_df %>% filter(is_strong_DEG), {
  list(
...
```{r left_panel_c_black_labels, fig.width=9, fig.height=7.5}
