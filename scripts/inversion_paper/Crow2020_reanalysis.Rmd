...
2. **Strong DEGs**: FDR < 0.05 AND appropriate |log2FC| threshold (`is_strong_DEG`)
...
# Load data
sra_meta <- read.csv(
  file.path(paths$data, "SraRunTable.csv"),
  stringsAsFactors = FALSE,
  check.names = FALSE
)
submission_meta <- read.csv(
  file.path(paths$data, "gc7_sample_submission.csv"),
  stringsAsFactors = FALSE
)
...
saveRDS(voomR$E, file = file.path(paths$intermediate, "normalized_expression_logCPM_crow2020.rds"))
saveRDS(voomR, file = file.path(paths$intermediate, "normalized_expression_voom_object_crow2020.rds"))
...
saveRDS(
  corrected_logCPM,
  file.path(paths$intermediate, "normalized_expression_batch_corrected_crow2020.rds")
)
...
effects_df <- effects_df %>%
  mutate(is_strong_DEG = is_DEG & is_large_effect)

{
  cat("\nStrong DEGs:\n")
  print(with(effects_df, table(predictor, is_strong_DEG)))
}
```
...
effects_df <- effects_df %>%
  group_by(is_strong_DEG, predictor, regulation) %>%
  mutate(
  ...
  ungroup() %>%
  mutate(
    is_selected_DEG = (pval_rank <= rank_threshold & is_strong_DEG) |
                      (mahal_rank <= rank_threshold & is_strong_DEG)
  )
...
overall_summary <- effects_df %>%
  ...
  summarise(
    total_genes = n(),
    n_DEG = sum(is_DEG),
    n_strong_DEG = sum(is_strong_DEG),
...
```{r check_trans_network_effects, eval=FALSE}
# NOTE: This chunk is disabled - network_effects.csv is not available
# and trans_effects is not used elsewhere in this script
...
write.csv(
  effects_df,
  file = file.path(paths$intermediate, "predictor_effects_crow2020.csv"),
  row.names = FALSE
)
...
write.csv(
  selected_degs,
  file = file.path(paths$intermediate, "selected_DEGs_crow2020.csv"),
  row.names = FALSE
)
...
# Concordance with Crow et al. 2020 Table S2

Cross-reference the 29 genes from Crow et al. Table S2 (three categories:
Consistent DE, Correlated Trans, TF) against our field DEGs. Requires
v4-to-v5 gene ID mapping since Crow used B73 v4 annotations.

```{r crow_concordance_setup}
# v4 to v5 gene ID mapping
v4_v5 <- read.table(
  here("data", "B73v4_to_B73v5.tsv"),
  header = FALSE, sep = "\t",
  col.names = c("v4", "v5_all")
) %>%
  mutate(v5 = sapply(strsplit(v5_all, ","), `[`, 1))

# Our field DEG results
field_effects <- read.csv(
  file.path(paths$intermediate, "predictor_effects_leaf_interaction_model.csv")
)

# Pannzer functional annotations
pannzer <- read.table(
  here("data", "PANNZER_DESC.tab"),
...
# Locus labels
labels <- read.csv(here("data", "locus_labels_master.csv")) %>%
  select(gene, locus_label)

# Crow Table S2
crow_s2 <- read.table(
  here("scripts", "00_agent_work", "Crow2020_table_S2.tsv"),
  header = TRUE, sep = "\t", fill = TRUE
) %>%
  left_join(v4_v5, by = c(Gene = "v4"))

cat("Crow Table S2:", nrow(crow_s2), "rows,",
    length(unique(crow_s2$Gene)), "unique v4 IDs\n")
cat("Mapped to v5:", sum(!is.na(crow_s2$v5)), "\n")
cat("No v5 mapping:", sum(is.na(crow_s2$v5)), "\n")
```

## Overall concordance

```{r crow_overall}
inv4m_eff <- field_effects %>%
  filter(predictor == "Inv4m") %>%
  select(gene, locus_label, logFC, adj.P.Val, is_DEG, is_strong_DEG,
         in_Inv4m, in_cis, in_flank, in_trans)

# Build full table: Crow genes with our effects
full <- crow_s2 %>%
  left_join(inv4m_eff, by = c(v5 = "gene")) %>%
  left_join(pannzer, by = c(v5 = "gene_model")) %>%
  mutate(conserved = !is.na(is_DEG) & is_DEG == TRUE)

# Deduplicate by v5 (some genes appear in multiple Crow categories)
dedup <- full %>%
  filter(!is.na(v5)) %>%
  distinct(v5, .keep_all = TRUE)

cat("Unique v5 genes:", nrow(dedup), "\n")
cat("DEGs in our study:", sum(dedup$conserved), "of", nrow(dedup), "\n")
cat("NOT DEGs:", sum(!dedup$conserved), "\n")
```

## Concordance by Crow category

```{r crow_by_category}
conserved <- full %>% filter(conserved)

for (cat_name in c("Consistent DE", "Correlated Trans", "TF")) {
  cat_rows <- full %>% filter(List == cat_name)
  cat_conserved <- cat_rows %>% filter(conserved)

  cat("\n===", cat_name, "===\n")
  cat("Total:", nrow(cat_rows), " | Conserved:", nrow(cat_conserved),
      " | Not conserved:", nrow(cat_rows) - nrow(cat_conserved), "\n")

  if (nrow(cat_conserved) > 0) {
    cat_conserved %>%
      mutate(direction = ifelse(logFC > 0, "UP", "DOWN")) %>%
      select(locus_label, v5, direction, logFC, adj.P.Val, desc) %>%
      arrange(logFC) %>%
      as.data.frame() %>%
      print()
  }
...
```{r crow_direction}
cat("Upregulated (logFC > 0):", sum(conserved$logFC > 0), "\n")
cat("Downregulated (logFC < 0):", sum(conserved$logFC < 0), "\n")
cat("Inside Inv4m:", sum(conserved$in_Inv4m, na.rm = TRUE), "\n")
cat("In shared introgression:", sum(conserved$in_cis, na.rm = TRUE), "\n")
cat("In trans:", sum(conserved$in_trans, na.rm = TRUE), "\n")
```

## Genes NOT conserved

```{r crow_not_conserved}
not_conserved <- full %>%
  filter(!is.na(v5)) %>%
  filter(!conserved)

cat("Not conserved genes with effect sizes in our data:\n")
not_conserved %>%
  select(List, v5, locus_label, logFC, adj.P.Val, Description) %>%
  arrange(adj.P.Val) %>%
  as.data.frame() %>%
  print()
```
