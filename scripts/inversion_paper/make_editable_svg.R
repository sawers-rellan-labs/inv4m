...
pairs <- list(
  list(top = "TIL18", bot = "PT",  inverted = FALSE),  # both inverted, no crossing
  list(top = "PT",    bot = "B73", inverted = TRUE))    # PT inverted vs B73 standard
...
# Save
# =============================================================================
outpath_svg <- file.path(paths$figures, "fig1_panel_B_skeleton.svg")
ggsave(outpath_svg, plot = make_final(FALSE), width = 9, height = 6, device = "svg",
       fix_text_size = TRUE)

outpath_png <- file.path(paths$figures, "fig1_panel_B_skeleton.png")
