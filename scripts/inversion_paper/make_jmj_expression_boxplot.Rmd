...
```{r boxplot, fig.width=12, fig.height=8}
# Define colors
genotype_colors <- c("B73" = "#009E73", "Inv4m" = "#D55E00")

p_jmj_expr <- ggplot(
 jmj_combined,
 aes(x = tissue, y = logCPM, fill = genotype)
) +
 geom_boxplot(
 ...
 labs(
   ...
   title = "JMJ Cluster Expression: Inv4m vs B73",
   subtitle = "Consistent downregulation of jmj2/jmj4 across tissues and experiments"
 )
...
```{r save_outputs}
# Save plot as RDS for composition
...
{
 cat("=== SAVED OUTPUTS ===\n")
 cat("jmj_expression_boxplot.rds (for composition)\n")
 cat("jmj_expression_boxplot.pdf\n")
 cat("jmj_expression_boxplot.png\n")
