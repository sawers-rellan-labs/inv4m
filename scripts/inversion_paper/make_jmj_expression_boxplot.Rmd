...
# SMO4 Expression (Supplementary Figure)

SMO4 (Zm00001eb192850) encodes a ribosome biogenesis protein NOP53. The Arabidopsis
ortholog *SMO4* regulates cell proliferation and organ size (Zhang et al. 2015 JIPB).
Downregulation in Inv4m (logFC = -1.98) provides an independent link to cell
proliferation control beyond MCM5/PCNA2.

## Define SMO4 Gene

```{r smo4_gene}
smo4_gene <- tibble(
  gene = "Zm00001eb192850",
  symbol = "smo4",
  description = "Ribosome biogenesis protein NOP53 (SMO4 ortholog)"
)
...
## Extract SMO4 Expression

```{r extract_smo4_psu2022}
# Extract SMO4 from PSU2022
smo4_in_psu <- smo4_gene$gene[smo4_gene$gene %in% rownames(expr_psu)]
cat("SMO4 found in PSU2022:", length(smo4_in_psu) > 0, "\n")

if (length(smo4_in_psu) > 0) {
  smo4_expr_psu <- expr_psu[smo4_in_psu, , drop = FALSE] %>%
    as.data.frame() %>%
    rownames_to_column("gene") %>%
    pivot_longer(
      cols = -gene,
  ...
  cat("PSU2022 SMO4 expression records:", nrow(smo4_expr_psu), "\n")
}
```

```{r extract_smo4_crow2020}
# Extract SMO4 from Crow2020
smo4_in_crow <- smo4_gene$gene[smo4_gene$gene %in% rownames(expr_crow)]
cat("SMO4 found in Crow2020:", length(smo4_in_crow) > 0, "\n")

if (length(smo4_in_crow) > 0) {
  smo4_expr_crow <- expr_crow[smo4_in_crow, , drop = FALSE] %>%
    as.data.frame() %>%
    rownames_to_column("gene") %>%
    pivot_longer(
      cols = -gene,
  ...
  cat("Crow2020 SMO4 expression records:", nrow(smo4_expr_crow), "\n")
}
```

```{r combine_smo4}
# Combine both experiments
smo4_combined <- bind_rows(
  smo4_expr_psu %>%
    dplyr::select(gene, symbol, logCPM, genotype, tissue, experiment),
  smo4_expr_crow %>%
    dplyr::select(gene, symbol, logCPM, genotype, tissue, experiment)
...
{
  cat("=== SMO4 COMBINED DATA ===\n")
  cat("Total records:", nrow(smo4_combined), "\n")
  cat("\nBy experiment:\n")
  print(table(smo4_combined$experiment))
}
```

## SMO4 Boxplot

```{r smo4_boxplot, fig.width=14, fig.height=5}
p_smo4_expr <- ggplot(
  smo4_combined,
  aes(x = tissue, y = logCPM, color = genotype)
) +
...
{
  cat("=== SMO4 EXPRESSION SUMMARY ===\n")
  print(
    smo4_summary %>%
      dplyr::select(experiment, tissue, symbol, log2FC, direction) %>%
      arrange(experiment, tissue)
  )
}
```
...
```{r save_smo4}
# Save SMO4 plot
saveRDS(p_smo4_expr, file.path(paths$figures, "smo4_expression_boxplot.rds"))
ggsave(
  file.path(paths$figures, "smo4_expression_boxplot.pdf"),
  p_smo4_expr,
  width = 14, height = 5
)
ggsave(
  file.path(paths$figures, "smo4_expression_boxplot.png"),
  p_smo4_expr,
...
write_csv(
  smo4_summary,
  file.path(paths$intermediate, "smo4_expression_summary.csv")
)

{
  cat("=== SMO4 SAVED OUTPUTS ===\n")
  cat("  smo4_expression_boxplot.rds/pdf/png\n")
  cat("  smo4_expression_summary.csv\n")
}
```
