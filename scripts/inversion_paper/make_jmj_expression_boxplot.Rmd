...
The chi-squared test is the appropriate test here because we are directly testing whether
observed proportions match expected proportions under the copy number hypothesis.
...
chisq_by_tissue <- total_cluster_samples %>%
  ...
  ungroup() %>%
  # Add FDR adjustment across all tissues

  mutate(
    chisq_fdr = p.adjust(chisq_p, method = "fdr")
  )

# Display as table
chisq_table <- chisq_by_tissue %>%
  dplyr::select(experiment, tissue, observed_ratio, chisq_stat, chisq_p, chisq_fdr, percent_of_expected) %>%
  mutate(
    observed_ratio = sprintf("%.2f:1", observed_ratio),
    chisq_stat = sprintf("%.2f", chisq_stat),
    chisq_p = sprintf("%.2e", chisq_p),
    chisq_fdr = sprintf("%.2e", chisq_fdr),
    percent_of_expected = sprintf("%.1f%%", percent_of_expected)
  )

knitr::kable(
  chisq_table,
  col.names = c("Experiment", "Tissue", "Observed Ratio", "χ²", "P-value", "FDR", "% of Expected"),
...
# Save per-tissue chi-square results with FDR
write_csv(
  chisq_by_tissue %>%
    dplyr::select(experiment, tissue, B73_total, Inv4m_total, observed_ratio,
                  chisq_stat, chisq_p, chisq_fdr, percent_of_expected),
  file.path(paths$intermediate, "jmj_total_cluster_chisq_by_tissue.csv")
)
