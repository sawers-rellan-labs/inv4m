...
## Statistical Tests (Per Tissue)

### Chi-Square Goodness of Fit Test

Test whether observed expression ratio matches expected copy number ratio (5:1) for each tissue.

```{r chisq_test_tissue}
# Chi-square test for each experiment AND tissue
chisq_by_tissue <- total_cluster_samples %>%
  group_by(experiment, tissue) %>%
  summarise(
  ...
  mutate(
    ...
    chisq_p = pchisq(chisq_stat, df = 1, lower.tail = FALSE),
    observed_ratio = B73_total / Inv4m_total,
    percent_of_expected = (Inv4m_total / expected_Inv4m) * 100
  ) %>%
  ungroup()

# Display as table
chisq_table <- chisq_by_tissue %>%
  dplyr::select(experiment, tissue, observed_ratio, chisq_stat, chisq_p, percent_of_expected) %>%
  mutate(
    observed_ratio = sprintf("%.2f:1", observed_ratio),
    chisq_stat = sprintf("%.2f", chisq_stat),
    chisq_p = sprintf("%.2e", chisq_p),
    percent_of_expected = sprintf("%.1f%%", percent_of_expected)
  )

knitr::kable(
  chisq_table,
  col.names = c("Experiment", "Tissue", "Observed Ratio", "χ²", "P-value", "% of Expected"),
  caption = "Chi-square goodness of fit test: H0 = expression ratio matches copy number (5:1)"
)
```

### One-Sample T-Test Against Expected Value

Test whether Inv4m expression differs from B73/5 (expected under copy number scaling) for each tissue.

```{r ttest_tissue}
# T-test for each experiment AND tissue
ttest_by_tissue <- total_cluster_samples %>%
  group_by(experiment, tissue) %>%
  summarise(
...
# Display as table
ttest_table <- ttest_by_tissue %>%
  dplyr::select(experiment, tissue, B73_mean, expected, Inv4m_mean, t_stat, df, ttest_p, percent_of_expected) %>%
  mutate(
    B73_mean = sprintf("%.1f", B73_mean),
    expected = sprintf("%.1f", expected),
    Inv4m_mean = sprintf("%.1f", Inv4m_mean),
    t_stat = sprintf("%.2f", t_stat),
    df = sprintf("%.0f", df),
    ttest_p = sprintf("%.2e", ttest_p),
    percent_of_expected = sprintf("%.1f%%", percent_of_expected)
  )

knitr::kable(
  ttest_table,
  col.names = c("Experiment", "Tissue", "B73 Mean", "Expected (B73/5)", "Inv4m Mean", "t", "df", "P-value", "% of Expected"),
  caption = "One-sample t-test: H0 = Inv4m mean equals B73/5"
)
```
...
# Save per-tissue statistical results
stats_by_tissue <- chisq_by_tissue %>%
  dplyr::select(experiment, tissue, observed_ratio, chisq_stat, chisq_p) %>%
  left_join(
    ttest_by_tissue %>%
      dplyr::select(experiment, tissue, t_stat, ttest_p, percent_of_expected),
    by = c("experiment", "tissue")
  )

write_csv(
  stats_by_tissue,
  file.path(paths$intermediate, "jmj_total_cluster_stats_by_tissue.csv")
)
