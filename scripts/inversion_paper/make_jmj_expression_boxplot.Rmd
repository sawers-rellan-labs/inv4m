...
```{r expected_values}
# Calculate B73 mean per experiment and tissue for expected lines
...
```{r total_cluster_boxplot, fig.width=14, fig.height=5}
...
expected_lines <- expected_by_tissue %>%
  mutate(genotype = factor("Inv4m", levels = c("B73", "Inv4m")))
...
p_total_cluster <- ggplot(
  ...
  with_shadow(colour = "black", x_offset = 0, y_offset = 0, sigma = 1) +
  # Add expected line (B73/5) for each tissue at the Inv4m position
  geom_errorbar(
    data = expected_lines,
  ...
  labs(
    ...
    caption = "Dashed black line = expected Inv4m (B73/5) under copy number scaling"
  )
