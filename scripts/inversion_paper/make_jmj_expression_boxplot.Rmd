...
**H₀: μ_B73 - 5×μ_Inv4m = 0** (expression scales with 5:1 copy number)
...
# H0: mean_B73 - 5*mean_Inv4m = 0 (expression scales with copy number)
contrast_by_tissue <- total_cluster_samples %>%
  group_by(experiment, tissue) %>%
  summarise(
    mean_B73 = mean(total_CPM[genotype == "B73"]),
    mean_Inv4m = mean(total_CPM[genotype == "Inv4m"]),
    var_B73 = var(total_CPM[genotype == "B73"]),
  ...
  mutate(
    # Observed ratio
    observed_ratio = mean_B73 / mean_Inv4m,
    # Expected Inv4m under copy number scaling
    expected_Inv4m = mean_B73 / 5,
    # Percent of expected
    percent_of_expected = (mean_Inv4m / expected_Inv4m) * 100,
    # Linear contrast: mean_B73 - 5*mean_Inv4m
    contrast = mean_B73 - 5 * mean_Inv4m,
    # Standard error of contrast (note: coefficient 5 gets squared for variance)
    se_contrast = sqrt(var_B73 / n_B73 + 25 * var_Inv4m / n_Inv4m),
    # t statistic
    t_stat = contrast / se_contrast,
    # Welch-Satterthwaite degrees of freedom
    df = (var_B73 / n_B73 + 25 * var_Inv4m / n_Inv4m)^2 /
         ((var_B73 / n_B73)^2 / (n_B73 - 1) + (25 * var_Inv4m / n_Inv4m)^2 / (n_Inv4m - 1)),
    # Two-tailed p-value
...
contrast_table <- contrast_by_tissue %>%
  dplyr::select(experiment, tissue, n_B73, n_Inv4m, observed_ratio, t_stat, df, p_value, fdr, percent_of_expected) %>%
  mutate(
...
knitr::kable(
  contrast_table,
  col.names = c("Experiment", "Tissue", "n B73", "n Inv4m", "Observed Ratio", "t", "df", "P-value", "FDR", "% of Expected"),
  caption = "Linear contrast t-test: H₀ = μ_B73 - 5×μ_Inv4m = 0 (expression scales with copy number)"
)
```
...
write_csv(
  contrast_by_tissue %>%
    dplyr::select(experiment, tissue, n_B73, n_Inv4m, mean_B73, mean_Inv4m,
                  observed_ratio, expected_Inv4m, percent_of_expected,
                  contrast, se_contrast, t_stat, df, p_value, fdr),
