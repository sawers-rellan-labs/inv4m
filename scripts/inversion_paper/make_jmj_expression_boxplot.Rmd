...
```{r expected_values}
# Calculate B73 mean per experiment (across all tissues) for expected line
...
Test whether observed expression ratio matches expected copy number ratio (5:1).

```{r chisq_test}
# Chi-square test for each experiment
chisq_results <- total_cluster_samples %>%
  group_by(experiment) %>%
  summarise(
    B73_total = sum(total_CPM[genotype == "B73"]),
    Inv4m_total = sum(total_CPM[genotype == "Inv4m"]),
    B73_n = sum(genotype == "B73"),
    Inv4m_n = sum(genotype == "Inv4m"),
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(
    # Expected ratio under copy number hypothesis: B73 should be 5x Inv4m
    # Expected Inv4m = (B73_total + Inv4m_total) / 6
    # Expected B73 = 5 * Expected Inv4m
    total_expression = B73_total + Inv4m_total,
    expected_Inv4m = total_expression / 6,
    expected_B73 = 5 * expected_Inv4m,
    # Chi-square statistic
    chisq_stat = (B73_total - expected_B73)^2 / expected_B73 +
                 (Inv4m_total - expected_Inv4m)^2 / expected_Inv4m,
    p_value = pchisq(chisq_stat, df = 1, lower.tail = FALSE),
    # Observed ratio
    observed_ratio = B73_total / Inv4m_total,
    # Actual Inv4m proportion of total
    Inv4m_proportion = Inv4m_total / total_expression,
    expected_proportion = 1/6,
    deficit = (expected_proportion - Inv4m_proportion) / expected_proportion * 100
  ) %>%
  ungroup()

{
  cat("=== CHI-SQUARE GOODNESS OF FIT TEST ===\n")
  cat("H0: Expression ratio matches copy number (5:1 B73:Inv4m)\n\n")
  for (i in 1:nrow(chisq_results)) {
    exp_name <- chisq_results$experiment[i]
    cat(sprintf("%s:\n", exp_name))
    cat(sprintf("  Observed ratio (B73:Inv4m): %.2f:1\n", chisq_results$observed_ratio[i]))
    cat(sprintf("  Expected ratio: 5:1\n"))
    cat(sprintf("  Chi-square statistic: %.2f\n", chisq_results$chisq_stat[i]))
    cat(sprintf("  P-value: %.2e\n", chisq_results$p_value[i]))
    cat(sprintf("  Inv4m produces %.1f%% of expected expression\n\n",
                100 - chisq_results$deficit[i]))
  }
}
```
...
Test whether Inv4m expression differs from B73/5 (expected under copy number scaling).

```{r ttest}
# T-test for each experiment
ttest_results <- total_cluster_samples %>%
  group_by(experiment) %>%
  summarise(
    B73_mean = mean(total_CPM[genotype == "B73"]),
    B73_sd = sd(total_CPM[genotype == "B73"]),
    Inv4m_mean = mean(total_CPM[genotype == "Inv4m"]),
    Inv4m_sd = sd(total_CPM[genotype == "Inv4m"]),
    Inv4m_values = list(total_CPM[genotype == "Inv4m"]),
    n_B73 = sum(genotype == "B73"),
  ...
  mutate(
    ...
    df = ttest$parameter,
    percent_of_expected = (Inv4m_mean / expected) * 100
  ) %>%
  ungroup() %>%
  dplyr::select(-Inv4m_values, -ttest)

{
  cat("=== ONE-SAMPLE T-TEST ===\n")
  cat("H0: Inv4m mean = B73 mean / 5\n\n")
  for (i in 1:nrow(ttest_results)) {
    exp_name <- ttest_results$experiment[i]
    cat(sprintf("%s:\n", exp_name))
    cat(sprintf("  B73 mean: %.2f CPM (n=%d)\n", ttest_results$B73_mean[i], ttest_results$n_B73[i]))
    cat(sprintf("  Expected Inv4m (B73/5): %.2f CPM\n", ttest_results$expected[i]))
    cat(sprintf("  Observed Inv4m mean: %.2f CPM (n=%d)\n", ttest_results$Inv4m_mean[i], ttest_results$n_Inv4m[i]))
    cat(sprintf("  t = %.3f, df = %.1f, p = %.2e\n",
                ttest_results$t_stat[i], ttest_results$df[i], ttest_results$p_value[i]))
    cat(sprintf("  Inv4m produces %.1f%% of expected expression\n\n",
                ttest_results$percent_of_expected[i]))
  }
}
```
...
p_total_cluster <- ggplot(
  ...
  with_shadow(colour = "black", x_offset = 0, y_offset = 0, sigma = 1) +
  # Add expected line (B73/5) for Inv4m
  geom_errorbar(
    data = expected_overall,
    aes(x = 2, ymin = expected_Inv4m, ymax = expected_Inv4m),
    inherit.aes = FALSE,
    width = 0.4,
    linewidth = 1,
    linetype = "dashed",
  ...
  labs(
    ...
    caption = "Dashed red line = expected Inv4m (B73/5) under copy number scaling"
  )
...
stats_combined <- bind_cols(
  chisq_results %>%
    dplyr::select(experiment, observed_ratio, chisq_stat, chisq_p = p_value),
  ttest_results %>%
    dplyr::select(t_stat, ttest_p = p_value, percent_of_expected)
)

write_csv(
  stats_combined,
  file.path(paths$intermediate, "jmj_total_cluster_stats.csv")
)
...

{
  ...
  cat("  jmj_total_cluster_stats.csv\n")
