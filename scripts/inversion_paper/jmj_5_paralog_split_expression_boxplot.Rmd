---
title: "JMJ Paralog Expression Boxplot — Supplementary Figure (PA2022, kallisto-corrected reference)"
...
**This script produces the per-paralog supplementary figure for the inversion
paper.** It analyzes JMJ cluster expression from transcript-level kallisto
quantification using the JMJ-corrected cDNA reference, where each of the five
paralogs is a distinct entry. This complements **Figure 7** (gene-level analysis
in `jmj_cluster_expression_boxplot.Rmd`), which uses the V5 annotation that
collapses jmj9+psi+jmj6+jmj2 into a single gene model `Zm00001eb191790`.

> **Symbol caveat.** In this notebook, "jmj2" refers to the single jmj2 paralog
> (`Zm00001eb191790_T013`). In the gene-level Figure 7 notebook, "jmj2" labels
> the V5 composite gene model `Zm00001eb191790` (= jmj9 + psi + jmj6 + jmj2 of
> the corrected reference). The two notebooks use the same symbol for different
> biological objects on purpose — this script is the only place where jmj2
> means a single paralog.

**Two analyses are performed:**

1. **Per-Paralog Expression** (the supplementary figure): Individual expression
   for each of the 5 paralogs. Because the paralogs are >97% identical, kallisto
   distributes Inv4m reads across all 5 loci artificially, so absolute per-paralog
   levels for Inv4m are uninterpretable. The plot is informative as a read-distribution
   diagnostic and to show that B73 reads concentrate in jmj2 + jmj4 (the V4 reference
   transcripts) while Inv4m reads spread across all five.

2. **Total Cluster Expression** (kallisto cross-check): Sum of CPM across all 5
   paralogs per sample. Methodological cross-check of the gene-level Figure 7
   total-cluster contrast — answers the same biological question with a different
   pipeline. Outputs are saved with `_kallisto` suffix to keep them distinct from
   the gene-level Figure 7 outputs.
...
# Save kallisto-based total cluster expression plot (log scale).
# Suffixed with `_kallisto` to keep distinct from the gene-level Figure 7 outputs
# in `jmj_cluster_expression_boxplot.Rmd` (`jmj_total_cluster_expression.*`).
saveRDS(p_jmj_total, file.path(paths$figures, "jmj_total_cluster_expression_kallisto.rds"))
ggsave(
  file.path(paths$figures, "jmj_total_cluster_expression_kallisto.pdf"),
  p_jmj_total,
  width = 8, height = 6
)
ggsave(
  file.path(paths$figures, "jmj_total_cluster_expression_kallisto.png"),
  p_jmj_total,
  width = 8, height = 6, dpi = 300
)

# Save kallisto-based total cluster expression plot (linear scale)
saveRDS(p_jmj_total_linear, file.path(paths$figures, "jmj_total_cluster_expression_kallisto_linear.rds"))
ggsave(
  file.path(paths$figures, "jmj_total_cluster_expression_kallisto_linear.pdf"),
  p_jmj_total_linear,
  width = 8, height = 6
)
ggsave(
  file.path(paths$figures, "jmj_total_cluster_expression_kallisto_linear.png"),
  p_jmj_total_linear,
  width = 8, height = 6, dpi = 300
)
...
# Save kallisto-based total cluster data (suffixed to keep distinct from gene-level)
write_csv(jmj_total,         file.path(paths$intermediate, "jmj_total_cluster_expression_kallisto_data.csv"))
write_csv(jmj_total_summary, file.path(paths$intermediate, "jmj_total_cluster_expression_kallisto_summary.csv"))

{
  cat("=== SAVED OUTPUTS ===\n")
  cat("\nPer-Paralog Analysis (the supplementary figure):\n")
  ...
  cat("\nKallisto-based Total Cluster (cross-check, distinct from Fig 7 outputs):\n")
  cat("  Figures: jmj_total_cluster_expression_kallisto.pdf/png/rds (log scale)\n")
  cat("  Figures: jmj_total_cluster_expression_kallisto_linear.pdf/png/rds (linear scale)\n")
  cat("  Data: jmj_total_cluster_expression_kallisto_data.csv\n")
  cat("  Data: jmj_total_cluster_expression_kallisto_summary.csv\n")
}
```
