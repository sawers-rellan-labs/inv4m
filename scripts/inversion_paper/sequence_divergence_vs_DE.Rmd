...
CDS percent identity from LAST pairwise alignments for `r nrow(merged)` expressed
...
genes (median `r med_inv4m`\% vs `r med_flank`\%;
Wilcoxon $p$ = `r wt_p`; Fig.~X A), consistent with
...
explained `r add_r2_pct`\% of the variance
in expression change (adj.\ $R^2$ = `r add_r2`;
divergence $p$ = `r add_p_div`;
region $p$ = `r add_p_loc`; Fig.~X B).
...
Notably, `r n_zero` genes had identical CDS between B73 and
Mi21, and all were significantly differentially expressed (FDR $<$ 0.05).
Of these, `r n_zero_flank` of `r n_zero` were located in the
flanking region, a significant overrepresentation
(Fisher's exact test, $p$ = `r ft_p`,
OR = `r ft_or`).
...
\begin{quote}
\textbf{Figure X.} CDS sequence divergence and expression change in the \textit{Inv4m}
introgression.
\textbf{(A)} Distribution of CDS divergence (100 $-$ \% identity) for expressed genes
inside the inversion (\textit{Inv4m}, $n$ = `r n_inv4m`) and in
the flanking introgression (flanking, $n$ = `r n_flank`).
Points show individual genes (beeswarm); boxes show median and interquartile range.
$y$-axis is log$_{10}$-scaled.
Wilcoxon rank-sum $p$ = `r wt_p`.
