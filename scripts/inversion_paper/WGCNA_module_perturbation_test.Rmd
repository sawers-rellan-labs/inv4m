...
```{r bootstrap_support_figure, fig.width=10, fig.height=10}
STABILITY_STABLE   <- 0.70
STABILITY_MODERATE <- 0.50

# Panel A: Genotype-response modules (original WGCNA colors)
p_gr_support <- ggplot(
  gr_lfp |> filter(color != "grey"),
  aes(x = reorder(color, mean_lfp), y = mean_lfp, fill = color)
) +
  ...
  labs(
    title = "Genotype-response modules",
    x = "Module", y = "Mean Support (LFP)"
  )

# Panel B: Leaf-gradient modules (remapped colors)
lg_lfp_remapped <- lg_lfp |>
  filter(color != "grey") |>
  mutate(
    display_color = lg_color_remap[color],
    fill_hex = lg_color_hex[display_color]
  )
...
p_lg_support <- ggplot(
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "Leaf-gradient modules",
    x = "Module", y = "Mean Support (LFP)"
  )

p_support_combined <- p_gr_support / p_lg_support +
  plot_annotation(tag_levels = "A")
print(p_support_combined)

ggsave(file.path(paths$figures, "module_support_combined.pdf"),
       p_support_combined, width = 10, height = 10)
ggsave(file.path(paths$figures, "module_support_combined.png"),
       p_support_combined, width = 10, height = 10, dpi = 300)
```
...
# Density ridgeline panel constructor
# points_df must have columns: module, z_observed, fdr
# fill_color: "identity" OR named vector (module -> hex)
# go_terms: optional named vector (module -> GO description)
...
```{r zscore_kwithin, fig.width=10, fig.height=10}
...
# Genotype-response density data
gr_density <- map_dfr(colnames(gr_null_z), function(m) {
  tibble(module = m, z_value = gr_null_z[, m])
})
gr_points <- tibble(
  module = names(gr_obs_z),
  z_observed = gr_obs_z,
  fdr = kw_fdr_gr[names(gr_obs_z)]
)
gr_order <- gr_points |> arrange(z_observed) |> pull(module)
gr_density$module <- factor(gr_density$module, levels = gr_order)
gr_points$module  <- factor(gr_points$module, levels = gr_order)

gr_mod_sizes <- sapply(levels(gr_density$module), function(m) {
  sum(gr_colors == m, na.rm = TRUE)
})

# Leaf-gradient density data
lg_density <- map_dfr(colnames(lg_null_z), function(m) {
  tibble(module = m, z_value = lg_null_z[, m])
})
lg_points <- tibble(
  module = names(lg_obs_z),
  z_observed = lg_obs_z,
  fdr = kw_fdr_lg[names(lg_obs_z)]
)
lg_order <- lg_points |> arrange(z_observed) |> pull(module)
lg_density$module <- factor(lg_density$module, levels = lg_order)
lg_points$module  <- factor(lg_points$module, levels = lg_order)

lg_mod_sizes <- sapply(levels(lg_density$module), function(m) {
  sum(lg_colors == m)
})

p_gr <- make_density_panel(gr_density, gr_points,
                           "Genotype-response modules (kWithin)",
                           fill_color = "identity",
                           go_terms = gr_go_terms,
                           module_sizes = gr_mod_sizes)
p_lg <- make_density_panel(lg_density, lg_points,
                           "Leaf-gradient modules (kWithin)",
                           fill_color = lg_color_hex,
                           go_terms = lg_go_terms,
                           module_sizes = lg_mod_sizes)

p_kw_combined <- p_gr / p_lg + plot_annotation(tag_levels = "A")
print(p_kw_combined)

ggsave(file.path(OUTPUT_DIR, "zscore_density_kWithin.pdf"),
       p_kw_combined, width = 10, height = 10)
ggsave(file.path(OUTPUT_DIR, "zscore_density_kWithin.png"),
       p_kw_combined, width = 10, height = 10, dpi = 300)
```
...
# Genotype-response kOut
gr_density_out <- map_dfr(colnames(gr_null_z_out), function(m) {
  tibble(module = m, z_value = gr_null_z_out[, m])
})
gr_points_out <- tibble(
  module = names(gr_obs_z_out),
  z_observed = gr_obs_z_out,
  fdr = kout_fdr_gr[names(gr_obs_z_out)]
)
gr_order_out <- gr_points_out |> arrange(z_observed) |> pull(module)
gr_density_out$module <- factor(gr_density_out$module, levels = gr_order_out)
gr_points_out$module  <- factor(gr_points_out$module, levels = gr_order_out)

# Leaf-gradient kOut
lg_density_out <- map_dfr(colnames(lg_null_z_out), function(m) {
  tibble(module = m, z_value = lg_null_z_out[, m])
})
lg_points_out <- tibble(
  module = names(lg_obs_z_out),
  z_observed = lg_obs_z_out,
  fdr = kout_fdr_lg[names(lg_obs_z_out)]
)
lg_order_out <- lg_points_out |> arrange(z_observed) |> pull(module)
lg_density_out$module <- factor(lg_density_out$module, levels = lg_order_out)
lg_points_out$module  <- factor(lg_points_out$module, levels = lg_order_out)

p_gr_kout <- make_density_panel(gr_density_out, gr_points_out,
                                "Genotype-response modules (kOut)",
                                fill_color = "identity",
                                go_terms = gr_go_terms,
                                module_sizes = gr_mod_sizes)
p_lg_kout <- make_density_panel(lg_density_out, lg_points_out,
                                "Leaf-gradient modules (kOut)",
                                fill_color = lg_color_hex,
                                go_terms = lg_go_terms,
                                module_sizes = lg_mod_sizes)

p_kout_combined <- p_gr_kout / p_lg_kout + plot_annotation(tag_levels = "A")
...
p_logfc <- ggplot(scatter_df, aes(x = abs_logFC, y = delta_kWithin, color = group)) +
  ...
  scale_color_manual(values = c("Genotype-response" = "grey40",
                                "Leaf-gradient" = "steelblue")) +
  theme_minimal(base_size = 14) +
  labs(
...
gr_only <- scatter_df |> filter(group == "Genotype-response")
...
# Leaf-gradient only
lg_only <- scatter_df |> filter(group == "Leaf-gradient")
...
```{r summary}
n_gr_sig <- sum(all_results$gene_set == "Genotype-response" & all_results$fdr < 0.05)
n_lg_sig <- sum(all_results$gene_set == "Leaf-gradient" & all_results$fdr < 0.05)

cat("=== PERMUTATION TEST SUMMARY ===

Genotype-response modules significant (FDR < 0.05):", n_gr_sig, "of",
    sum(all_results$gene_set == "Genotype-response"), "
Leaf-gradient modules significant (FDR < 0.05):", n_lg_sig, "of",
    sum(all_results$gene_set == "Leaf-gradient"), "

Significant genotype-response modules:\n")
all_results |>
  filter(gene_set == "Genotype-response", fdr < 0.05) |>
  dplyr::select(module, n_genes, observed_mean_delta_k, ecdf_pvalue, fdr) |>
  print()
