...
```{r bootstrap_support_figure, fig.width=10, fig.height=8}
...
# Order modules by z_observed within each gene_set
mod_order <- points_kw |> arrange(gene_set, z_observed) |> pull(module)
...
gr_go_df <- if (!is.null(gr_go_terms)) {
  tibble(
    ...
    go_desc = unname(gr_go_terms), gene_set = "Genotype-response"
...
lg_go_df <- if (!is.null(lg_go_terms)) {
  tibble(
    ...
    go_desc = unname(lg_go_terms), gene_set = "Leaf-gradient"
  ) |> filter(module %in% levels(density_kw$module))
}
...
p_kw <- ggplot(density_kw, aes(x = z_value, y = module, fill = module)) +
  ...
  facet_wrap(~gene_set, scales = "free_y", ncol = 1) +
  theme_minimal(base_size = 14) +
  labs(y = NULL, x = expression(Z[Delta * k[Within]]))

print(p_kw)

ggsave(file.path(OUTPUT_DIR, "zscore_density_kWithin.pdf"),
       p_kw, width = 10, height = 10)
...
# Order modules by z_observed within each gene_set
mod_order_out <- points_kout |> arrange(gene_set, z_observed) |> pull(module)
