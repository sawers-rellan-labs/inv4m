...
# All gene_symbol.tab aliases (for expanding multi-alias genes like jmj2/jmj6/jmj9)
...
# expand genes with multiple aliases in gene_symbol.tab
annotate_and_expand <- function(df) {
  # First, supplement description: prefer desc (PANNZER functional) over desc_merged
  df <- df %>%
    mutate(
  ...
  # Split into genes with aliases and those without
  genes_with_aliases <- df %>%
    inner_join(
      gene_symbol_aliases %>% dplyr::select(gene_model, alias = locus_symbol),
      by = c("gene" = "gene_model"),
      relationship = "many-to-many"
    ) %>%
    # Use alias as label, keep everything else
    mutate(locus_symbol = alias) %>%
    dplyr::select(-alias)

  genes_without_aliases <- df %>%
    dplyr::filter(!(gene %in% gene_symbol_aliases$gene_model)) %>%
    mutate(locus_symbol = ifelse(is.na(locus_symbol) | locus_symbol == "",
                                 "", locus_symbol))

  bind_rows(genes_with_aliases, genes_without_aliases)
}
