...
```{r method2_dotplot, fig.height=10, fig.width=12}
if (nrow(m2_inv4m %>% filter(p.adjust < 0.05)) > 0) {
  dotplot(compGO_BP_M2, size = "Count", showCategory = 10) +
    ggtitle("GO Biological Process: Method 2 (Strongest Non-Flanking Regulator)") +
    scale_size(range = c(4, 8), breaks = 1:4) +
    theme(axis.text.x = element_text(angle = 30, hjust = 1))
} else {
  cat("No significant GO terms for Method 2 Inv4m sets\n")
}
```
