...
- **Strong DEGs**: for GO enrichment, FDR < 0.05 AND $\log_2FC > \pm1.5$ (`is_strong_DEG`).
...
saveRDS(transcript_MDS_34, file.path(paths$intermediate, "transcript_MDS_34.rds"))
...
# RNA_Plant is already a unique plant identifier in metadata
cat("Plant blocking structure:\n")
cat("  Unique plants:", length(unique(d$RNA_Plant)), "\n")
print(table(d$RNA_Plant))
...
corfit <- duplicateCorrelation(voomR, design, block = d$RNA_Plant)
...
fit <- lmFit(voomR, design, block = d$RNA_Plant, correlation = corfit$consensus.correlation)
...
{
  cat("\nStrong DEGs (is_strong_DEG) Count:\n")
  print(with(effects_df, table(predictor, is_strong_DEG)))
  print(
    with(
      effects_df %>% filter(is_strong_DEG),
      table(predictor, regulation, is_strong_DEG)
    ) %>%
      as_tibble() %>%
...
# Marker types (from data/maizegdb_consensus_map/):
...
effects_df <- effects_df %>%
  mutate(
    locus_label = case_when(
      ...
      # If locus_name exists, the symbol is a real gene name
      !is.na(locus_name) & locus_name != "" ~ locus_symbol,
      # If no locus_name and matches marker pattern, filter it
      grepl(marker_pattern, locus_symbol) ~ NA_character_,
...
## Strong DEG Enrichment

```{r fisher_tests_strong_degs}
strong_deg_tests <- bind_rows(
  run_fisher(Region_effects, "in_cis", "is_strong_DEG") %>%
    mutate(comparison = "Shared introgression vs Outside"),
  run_fisher(
    Region_effects %>% filter(in_flank | outside),
    "in_flank",
...
{
  cat("\nStrong DEG Enrichment (FDR < 0.05, |logFC| > 1.5):\n")
  print(strong_deg_tests)
}
```
...
```{r enrichment_interpretation}
{
  ...
  cat("3. Regional enrichment pattern observed for strong DEGs\n")
...
# Define genes of interest - strong interaction genes
interaction_highconf <- effects_df %>%
  filter(is_strong_DEG & predictor == "Leaf:-P")
...
## LaTeX table: Inv4m gene effects (Table S3)

```{r inv4m_gene_effects_latex}
# Prepare Inv4m DEGs + Leaf×Inv4m interaction for LaTeX table
inv4m_degs <- selected_degs %>%
  filter(predictor == "Inv4m") %>%
  arrange(regulation, -neglogP)

interaction_degs <- selected_degs %>%
  filter(predictor == "Leaf:Inv4m") %>%
  arrange(-neglogP)

# Row formatter (same pattern as pink module table)
format_gene_effect_row <- function(gene, label, desc, log2fc, neglogp, adj_p_val) {
  label_fmt <- if (is.na(label) || label == "" || label == gene) "" else paste0("\\textit{", label, "}")
  desc_clean <- if (is.na(desc) || desc == "") "" else {
    stringr::str_trunc(desc, 40) %>%
      stringr::str_replace_all("_", "\\\\_") %>%
      stringr::str_replace_all("&", "\\\\&") %>%
      stringr::str_replace_all("%", "\\\\%")
  }
  log2fc_fmt <- if (log2fc >= 0) paste0("$+", sprintf("%.2f", log2fc), "$") else paste0("$", sprintf("%.2f", log2fc), "$")
  neglogp_fmt <- sprintf("%.1f", neglogp)
  paste(gene, label_fmt, neglogp_fmt, log2fc_fmt, desc_clean, sep = " & ") %>% paste0(" \\\\")
}

# Generate rows
down_rows <- inv4m_degs %>%
  filter(regulation == "Downregulated") %>%
  rowwise() %>%
  mutate(row = format_gene_effect_row(gene, locus_label, desc_merged, logFC, neglogP)) %>%
  pull(row)
...
latex_table <- paste(
  latex_header,
  paste(down_rows, collapse = "\n"),
  latex_middle,
  paste(up_rows, collapse = "\n"),
  latex_interaction,
...
writeLines(latex_table, file.path(paths$tables, "gene_effects.tex"))
cat("LaTeX table saved: tables/gene_effects.tex\n")
```
...
- **Strong DEGs** (FDR < 0.05, |logFC| > 1.5): 2937 genes
