...
bar_y0 <- 100; bar_h <- 10  # bar from y=100 to y=110
...
make_panel <- function(genome_id, show_points = FALSE, bot_margin = 5,
  ...
  p <- ggplot() +
    ...
    scale_x_continuous(labels = unit_format(unit = "", scale = 1e-6),
                       breaks = cfg$xbreaks) +
    scale_y_continuous(breaks = c(0, 25, 50, 75, 100), limits = c(-13, 113)) +
...
build_overlay <- function(genome_labels) {
  ...
  xtick_annots <- lapply(names(xtick_x), function(lab) {
    draw_label(lab, x = xtick_x[lab] / svg$W, y = y_xtick - 0.02,
  ...
  c(list(
    ...
    draw_label("Inv4m", x = 0.339, y = 0.895, size = sz_name,
               fontface = "bold.italic", color = col_inv4m),
    legend_dot("gray50", 0.55, y_header - 0.012),
    draw_label("knob 180", x = 0.58, y = y_header, size = sz_species,
               fontface = "bold.italic", color = "gray50", hjust = 0),
    legend_dot("gray80", 0.73, y_header - 0.012),
...
variant_mi21 <- list(
  ...
  vlines = list(
    TIL18 = list(up_x = 200.20, dn_x = 293.11, top_y = 72.02,  bot_y = 202.20),
    Mi21  = list(up_x = 200.20, dn_x = 276.67, top_y = 222.13, bot_y = 352.31),
    B73   = list(up_x = 200.20, dn_x = 307.49, top_y = 372.24, bot_y = 505.41)),
