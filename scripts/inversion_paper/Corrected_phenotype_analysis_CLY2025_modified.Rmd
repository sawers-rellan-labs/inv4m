---
title: "NC2025 Spatial Analysis: SpATS Spatial Correction with GDD Integration"
...
1. Load GDD lookup table for NC2025.
2. Load and clean plant-level data from NC2025.
3. **Aggregate to plot-level means** (matching PA2022 structure).
...
# GDD lookup table from inversion_paper intermediate outputs
gdd_lookup_file <- file.path(paths$intermediate, "gdd_lookup_table.csv")

# Check if GDD lookup exists; if not, we'll use GDD columns from raw data
if (file.exists(gdd_lookup_file)) {
  ...
  cat("GDD lookup table not found. Will use DTA_GDD/DTS_GDD columns from raw data.\n")
  use_gdd_lookup <- FALSE
}
```
...
```{r load-data}
file_path <- file.path(paths$data, "CLY25_Inv4m.csv")
if (!file.exists(file_path)) {
  stop("Error: CLY25_Inv4m.csv not found in data directory: ", paths$data)
}
...
# Add GDD columns: use lookup table if available, otherwise use raw data columns
if (use_gdd_lookup) {
  ...
  # Use existing GDD columns from raw data (DTA_GDD, DTS_GDD)
  field_data <- field_data %>%
    rename(GDDA = DTA_GDD, GDDS = DTS_GDD)
}
  ...
  if (length(corrected_phenotypes) > 0) {
    ...
    for (current_donor in donors) {
      ...
      diff_plot <- ggplot(plot_data_donor, 
        ...
        labs(
          title = paste("NC2025: ", current_donor,
...
## 9. Statistical comparisons and visualization for NC2025
...
if (exists("sp_corrected") && nrow(sp_corrected) > 0) {
  ...
  if (length(trait_plots) > 0) {
    ...
    combined_plot <- ggarrange(
      ...
      annotate_figure(
        top = text_grob("NC2025 — Spatially Corrected Phenotypes",
                       color = "black", 
                       face = "bold", 
                       size = 18)
      )
  ...
  message("No spatially corrected data available for NC2025")
}
```
...
if (exists("sp_corrected") && nrow(sp_corrected) > 0) {
  ...
  # Export to inversion_paper intermediate (used by GxE analysis)
  output_file <- file.path(paths$intermediate,
