...
# Load curated labels
curated <- read.csv(
  file.path(paths$data, "selected_DEGs_curated_locus_label_2.csv"),
  na.strings = c("NA", NA, "")
) %>%
...
jmj4_neighborhood_labels <- c(
  "jmj4", "engd2", "roc4-2", "kan1", "sec6", "nii2", "tet2", "flz22", "pza01976"
)

# Get gene IDs matching these labels
jmj4_genes_by_label <- effects_df %>%
  filter(locus_label %in% jmj4_neighborhood_labels) %>%
  pull(gene) %>%
  unique()

cat("jmj4 neighborhood genes:", length(jmj4_genes_by_label), "\n")

if (nrow(novel_edges_prep) > 0 && length(jmj4_genes_by_label) > 0) {
  ...
  jmj4_nodes_in_mst <- V(g_novel_mst)$name[V(g_novel_mst)$name %in% jmj4_genes_by_label]
