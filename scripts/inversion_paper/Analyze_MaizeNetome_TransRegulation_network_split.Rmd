...
output:
  html_document:
    ...
    theme: flatly
    df_print: paged
knit: (function(input, ...) { rmarkdown::render(input, output_dir = "../../results/phosphorus_paper") })
---
...
v5_gff <- "data/zea/Zea_mays.Zm-B73-REFERENCE-NAM-5.0.59.chr.gff3"
...
```{r load_field_degs}
effects_df <- read.csv(
  "results/phosphorus_paper/intermediate/predictor_effects_leaf_interaction_model.csv", 
  na.strings = c("NA", NA, "")
) %>%
  filter(predictor == "Inv4m")

curated <- read.csv("data/selected_DEGs_curated_locus_label_2.csv", 
                    na.strings = c("NA", NA, "")) %>%
...
```{r load_maizenetome}
left_flanking_maizenetome <- read.csv("data/left_flanking_maizenetome.csv")

Inv4m_maizenetome <- read.csv("data/Inv4m_maizenetome.csv")

right_flanking_maizenetome <- read.csv("data/right_flanking_maizenetome.csv")
...
voomR <- readRDS("results/phosphorus_paper/intermediate/normalized_expression_voom_object_leaf_trt.rda")
...
linked <- read.table("data/inv4m_linked_genes_to_filter.txt", header = TRUE)
...
write.csv(
    ...
    "~/Desktop/inv4m_bipartite_edges.csv",
    row.names = FALSE
)
